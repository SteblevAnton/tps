...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПриСозданииНаСервереУХ(ЭтотОбъект, Отказ, СтандартнаяОбработка);
	#КонецОбласти
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИсточникВыбора.ИмяФормы = "Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.ФормаВыбораУпаковкиЕдиницы" Тогда
	...
	Иначе
		...
		Если Элементы.Помощник.ТекущаяСтраница = Элементы.ШагРедактированиеЗаказов Тогда
			...
			Если РедактируемаяСтрока.УровеньВДереве = 0 И РедактируемаяСтрока.ТипОбеспечения = Покупка Тогда
				...
				ТекущийИндекс = 0;
				Пока ТекущийИндекс < СтрокиЗаказа.Количество() Цикл
					СтрокаДерева = СтрокиЗаказа[ТекущийИндекс];
...
Процедура ФильтрУровеньЗапасаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.ФормаВыбораУровняЗапаса", ПараметрыФормы, ЭтотОбъект, УникальныйИдентификатор);
...
Процедура ЗапасыПриАктивизацииСтроки(Элемент)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.Элемент.Запасы.ПриАктивизацииСтроки");
...
Процедура ЗапасыПриАктивизацииЯчейки(Элемент)
	...
	// НАЧАЛО Османов Н.М. 18.04.2025 
	Если СтрокаЗапасы = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(СтрокаЗапасы.ЗаказыКПоступлению) Тогда
		РасшифровкаЗаказов.ПолучитьЭлементы().Очистить();
		Возврат;
	КонецЕсли;
	
	СтруктураОтбора = Новый Структура("Номенклатура, Характеристика");
	ЗаполнитьЗначенияСвойств(СтруктураОтбора, СтрокаЗапасы);
	ЗаполнитьТаблицуРасшифровки(СтруктураОтбора);
	
	Для Каждого ЭлементДереваЗаказов Из РасшифровкаЗаказов.ПолучитьЭлементы() Цикл
		Элементы.РасшифровкаЗаказов.Развернуть(ЭлементДереваЗаказов.ПолучитьИдентификатор(), Истина);
	КонецЦикла;
	// КОНЕЦ Османов Н.М. 18.04.2025 
...
Процедура ОтметкаЗапасыПриИзменении(Элемент)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.Элемент.ОтметкаЗапасы.ПриИзменении");
...
Процедура КЗаказуЗапасыПриИзменении(Элемент)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.Элемент.КЗаказуЗапасы.ПриИзменении");
...
Процедура СпособОбеспеченияЗапасыПриИзменении(Элемент)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.Элемент.СпособОбеспеченияЗапасы.ПриИзменении");
...
Процедура ИсточникОбеспеченияЗапасыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.Форма.Элемент.ИсточникОбеспеченияЗапасы.НачалоВыбора");
	...
	СтандартнаяОбработка = Ложь;
	ОткрытьФорму("Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.ФормаВыбораИсточникаОбеспечения", ПараметрыФормы, ЭтотОбъект, УникальныйИдентификатор);
...
Процедура ДатаПоставкиЗапасыПриИзменении(Элемент)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.Элемент.ДатаПоставкиЗапасы.ПриИзменении");
...
Процедура ЗаказыКОбеспечениюПриАктивизацииСтроки(Элемент)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.Элемент.ЗаказыКОбеспечению.ПриАктивизацииСтроки");
...
Процедура ОтметкаЗаказыПриИзменении(Элемент)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.Элемент.ОтметкаЗаказы.ПриИзменении");
...
Процедура КЗаказуЗаказыПриИзменении(Элемент)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.Элемент.КЗаказуЗаказы.ПриИзменении");
...
Процедура СпособОбеспеченияЗаказыПриИзменении(Элемент)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.Элемент.СпособОбеспеченияЗаказы.ПриИзменении");
...
Процедура ИсточникОбеспеченияЗаказыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.Форма.Элемент.ИсточникОбеспеченияЗаказы.НачалоВыбора");
...
Процедура ДатаПоставкиЗаказыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Поставка.Отметка Тогда
		...
		Если Поставка.ТипОбеспечения = ПредопределенноеЗначение("Перечисление.ТипыОбеспечения.Покупка") 
			...
			ОтразитьПоставкуСогласноПланамПоставкиВКорзине(ЭтотОбъект, Поставка, 0, Истина);
		Иначе
 
		ОтразитьПоставкуВКорзине(ЭтотОбъект, Поставка, 0, Истина);
	...
	Если Поставка.ТипОбеспечения = ПредопределенноеЗначение("Перечисление.ТипыОбеспечения.Покупка") 
		И Поставка.СогласноПланамПоставокПоДоговорам Тогда
		ОтразитьПоставкуСогласноПланамПоставкиВКорзине(ЭтотОбъект, Поставка, КоличествоКЗаказу, Истина);
	Иначе

	ОтразитьПоставкуВКорзине(ЭтотОбъект, Поставка, КоличествоКЗаказу, Истина);
...
Процедура ДатаПоставкиЗаказыПриИзменении(Элемент)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.Элемент.ДатаПоставкиЗаказы.ПриИзменении");
...
Процедура ПредварительныйПросмотрВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ПредварительныйПросмотрКЗаказу"
		...
		ОткрытьФорму("Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.ФормаРедактированияКоличества", ПараметрыДляФормыРедактирования,,,,,ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОткрытьВExcel(Команда)
	...
	ТабличныйДокументЗапасы = СформироватьТабличныйДокументЗапасы();
	...
	ТабличныйДокументЗапасы.Записать(ИмяФайла, ТипФайлаТабличногоДокумента.XLSX);
...
Процедура Назад(Команда)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.Команда.Назад");
...
Процедура НазадФрагмент()
	...
	ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация,
		Новый ОписаниеОповещения("УдалениеЗаказовВФонеЗавершение", ЭтотОбъект),
		ПараметрыОжидания);
...
Процедура Далее(Команда)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.Команда.Далее");
	...
	Если Элементы.Помощник.ТекущаяСтраница = Элементы.ШагОтборПотребностей Тогда
		...
		ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация,
			Новый ОписаниеОповещения("ПолучениеТаблицыСпособовОбеспеченияВФонеЗавершение", ЭтотОбъект),
			ПараметрыОжидания);
	...
	ИначеЕсли Элементы.Помощник.ТекущаяСтраница = Элементы.ШагНастройкаСпособовОбеспечения И ЕстьШагЗапасы Тогда
		...
		Если ТаблицаСпособовЗаполненаВерно() Тогда
			...
			ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация,
				Новый ОписаниеОповещения("ПолучениеТаблицыЗапасовВФонеЗавершение", ЭтотОбъект),
				ПараметрыОжидания);
	...
	ИначеЕсли Элементы.Помощник.ТекущаяСтраница = Элементы.ШагНастройкаСпособовОбеспечения И Не ЕстьШагЗапасы
		Или Элементы.Помощник.ТекущаяСтраница = Элементы.ШагиПотребности
			...
			ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация,
				Новый ОписаниеОповещения("ПолучениеТаблицыЗаказовКОбеспечениюВФонеЗавершение", ЭтотОбъект),
				ПараметрыОжидания);
	...
	ИначеЕсли Элементы.Помощник.ТекущаяСтраница = Элементы.ШагиПотребности
		И Элементы.СтраницыТаблицаПотребностей.ТекущаяСтраница = Элементы.СтраницаТаблицаЗаказы Тогда
			...
			ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация,
				Новый ОписаниеОповещения("ПереходКРедактированиюЗаказовЗавершение", ЭтотОбъект),
				ПараметрыОжидания);
...
Процедура НастроитьПоддержаниеЗапаса(Команда)
	...
	ОткрытьФорму("Обработка.НастройкаПоддержанияЗапасов.Форма.Форма", ПараметрыФормы, ЭтотОбъект, УникальныйИдентификатор);
...
Процедура НастроитьРасчетСтатистикиПотребления(Команда)
	...
	ОткрытьФорму("Обработка.НастройкаПоддержанияЗапасов.Форма.НастройкаПараметровРасчетаСреднедневногоПотребления",, ЭтотОбъект);
...
Процедура ЗаполнитьДатуПоставки(Команда)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.Форма.Команда.ЗаполнитьДатуПоставки");
...
Функция ПолучениеТаблицыСпособовОбеспеченияВФоне()
	...
	НастройкиКомпоновки = КомпоновщикНастроекДляОтборов.ПолучитьНастройки();
	
	// НАЧАЛО Османов Н.М. 18.04.2025 
	ОтключитьИспользованиеПоОтбору(НастройкиКомпоновки, "СкладыОстатков");
	ОтключитьИспользованиеПоОтбору(НастройкиКомпоновки, "СкладыОбеспечения");
	ОтключитьИспользованиеПоОтбору(НастройкиКомпоновки, "СкладыПроизводства");
	ОтключитьИспользованиеПоОтбору(НастройкиКомпоновки, "НачалоПериодаЗакупок");
	ОтключитьИспользованиеПоОтбору(НастройкиКомпоновки, "ОкончаниеПериодаЗакупок");
	// КОНЕЦ Османов Н.М. 18.04.2025 
	// НАЧАЛО Османов Н.М. 13.05.2025 
	ОтключитьИспользованиеПоОтбору(НастройкиКомпоновки, "НачалоИсторииПотребления");
	ОтключитьИспользованиеПоОтбору(НастройкиКомпоновки, "ОкончаниеИсторииПотребления");
	// КОНЕЦ Османов Н.М. 13.05.2025 
	
	ПараметрыПроцедуры.Вставить("Настройки", НастройкиКомпоновки);
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("Обработки.ОбеспечениеПотребностейНоваяСхема.ТаблицаСпособовОбеспечения",
...
Функция ПолучениеТаблицыЗапасовВФоне()
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("Обработки.ОбеспечениеПотребностейНоваяСхема.ТаблицаЗапасы",
		ПараметрыПроцедуры, ПараметрыВыполнения);
...
Функция ПолучениеТаблицыЗаказовКОбеспечениюВФоне()
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("Обработки.ОбеспечениеПотребностейНоваяСхема.ТаблицаПотребностейПоЗаказамИОстатков",
		ПараметрыПроцедуры, ПараметрыВыполнения);
...
Функция ФормированиеДереваЗаказовВФоне()
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("Обработки.ОбеспечениеПотребностейНоваяСхема.СформироватьДеревоЗаказов",
		ПараметрыПроцедуры, ПараметрыВыполнения);
...
Функция ФормированиеЗаказовВФоне()
	...
	Поля = Обработки.ОбеспечениеПотребностейНоваяСхема.ПоляТаблицыЗаказы(СекцииДляВыгрузки);
	...
	#КонецОбласти 	
	Возврат ДлительныеОперации.ВыполнитьВФоне("Обработки.ОбеспечениеПотребностейНоваяСхема.СформироватьЗаказы",
		ПараметрыПроцедуры, ПараметрыВыполнения);
...
Функция УдалениеЗаказовВФоне()
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("Обработки.ОбеспечениеПотребностейНоваяСхема.УдалитьЗаказы",
		ПараметрыПроцедуры, ПараметрыВыполнения);
...
Процедура ЗаполнитьРеквизитыСпособаОбеспеченияНаСервере(СтруктураРеквизитов, НачалоПериода)
	...
	Обработки.ОбеспечениеПотребностейНоваяСхема.ЗаполнитьРеквизитыСпособаОбеспечения(СтруктураРеквизитов, НачалоПериода, Ложь);
...
Процедура ЗаписатьИзмененияСпособовОбеспечения()
	...
	|	ГДЕ
	|		НЕ СпрСпособ.ФормироватьЗаказыПоРасписанию
	|		ИЛИ (ТаблицаОтбора.ПлановаяДатаПоставки  <> СпрСпособ.ПерваяДатаВГрафикеПоставок
	|			И ТаблицаОтбора.ПлановаяДатаПоставки <> ДАТАВРЕМЯ(1, 1, 1))
	|		ИЛИ ТаблицаОтбора.ДатаСледующейПоставки <> СпрСпособ.УдалитьДатаСледующейПоставки
	|		ИЛИ СпрСпособ.УдалитьПлановаяДатаЗаказа <> &НачалоПериода
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////";
	...
	Если ПравоДоступа("Изменение", Метаданные.Справочники.СпособыОбеспеченияПотребностей)
		...
		Пока Выборка.Следующий() Цикл
			...
			СпособОбъект.УдалитьДатаСледующейПоставки  = Выборка.ДатаСледующейПоставки;
			СпособОбъект.ПерваяДатаВГрафикеПоставок    = Выборка.ПлановаяДатаПоставки;
			СпособОбъект.УдалитьПлановаяДатаЗаказа     = НачалоПериода;
			СпособОбъект.ФормироватьЗаказыПоРасписанию = Истина;
...
Процедура РассчитатьСтрокуТаблицыЗапасыНаКлиенте(СпособОбеспечения)
	...
	КонецЕсли;
    //++ ШмелевР 16.02.2026 ЗКП.1.2
	Отбор = Новый Структура("Номенклатура, Характеристика, Склад, СпособОбеспечения, ДатаПоставки, ФормироватьПлановыеЗаказы");
	//-- ШмелевР 16.02.2026 ЗКП.1.2
...
Процедура РассчитатьСтрокуТаблицыЗапасы(ИдентификаторСтроки, Отбор)
	...
	//++ ШмелевР 16.02.2026 ЗКП.1.2	
		АдресХранилищаСКДПользовательскиеНастройки = ПоместитьВоВременноеХранилище(КомпоновщикНастроекДляОтборов.ПользовательскиеНастройки, Новый УникальныйИдентификатор);
	Обработки.ОбеспечениеПотребностейНоваяСхема.ТаблицаЗапасы(
		Новый Структура("ТипОтбора, КлючДанных, Отбор", "ОтборПоЗначениямСФормы", АдресХранилищаСКДПользовательскиеНастройки, Отбор),
		АдресХранилища);
	//-- ШмелевР 16.02.2026 ЗКП.1.2 
	...
	//++ ШмелевР 20.02.2026 ЗКП.1.2
	СтрокиКРедактированию = Таблица.НайтиСтроки(Отбор);
	Если СтрокиКРедактированию.Количество() > 0 Тогда
		СтрокаКРедактированию = СтрокиКРедактированию[0];
	Иначе
		СтрокаКРедактированию = Неопределено;
	КонецЕсли;
	//-- ШмелевР 20.02.2026 ЗКП.1.2
	Если Таблица.Количество() > 0 Тогда
		
		//++ ШмелевР 20.02.2026 ЗКП.1.2
		Если СтрокаКРедактированию <> Неопределено Тогда
			Товар = СтрокаКРедактированию;	
		Иначе
			Товар = Таблица[0];	
		КонецЕсли;
		//-- ШмелевР 20.02.2026 ЗКП.1.2
		...
		РезультатРасчета = Обработки.ОбеспечениеПотребностейНоваяСхема.МинимальныйИМаксимальныйЗапасы(СтрокаЗапасы);
		ЗаполнитьЗначенияСвойств(СтрокаЗапасы, РезультатРасчета);
	
		РезультатРасчета = Обработки.ОбеспечениеПотребностейНоваяСхема.КоличествоКЗаказуДляПоддержанияЗапаса(СтрокаЗапасы);
		ЗаполнитьЗначенияСвойств(СтрокаЗапасы, РезультатРасчета);
	
		РезультатРасчета = Обработки.ОбеспечениеПотребностейНоваяСхема.УровеньЗапасаИКартинкаСменитьМетод(СтрокаЗапасы);
		ЗаполнитьЗначенияСвойств(СтрокаЗапасы, РезультатРасчета);  
		...
		//++ ШмелевР 16.02.2026 ЗКП.1.2
		// ЗКП.КЗаказу
		МетодПоддержанияПоСтатистике = Перечисления.МетодыОбеспеченияПотребностей.ПоддержаниеЗапасаНаСрокПоСтатистике;
		Если СтрокаЗапасы.МетодОбеспечения = МетодПоддержанияПоСтатистике Тогда
			ДоТочкиЗаказаКПерерасчету = СтрокаЗапасы.Остаток - СтрокаЗапасы.МинимальныйЗапас;
			ВосполнениеСтраховогоЗапаса = Мин(СтрокаЗапасы.СтраховойЗапас, Макс(0, -ДоТочкиЗаказаКПерерасчету));
			
			СтрокаЗапасы.КЗаказу = Макс(0, (СтрокаЗапасы.МаксимальныйЗапас - СтрокаЗапасы.СтраховойЗапас - СтрокаЗапасы.ЗаказыКпоступлению - Макс(0, ДоТочкиЗаказаКПерерасчету)) * Товар.cbr_ПроцентКЗакупке / 100 + ВосполнениеСтраховогоЗапаса) + (Макс(0, Мин(СтрокаЗапасы.РезервыНаСкладе, -СтрокаЗапасы.Остаток)));
			РезультатРасчетаДляОтметки = Обработки.ОбеспечениеПотребностейНоваяСхема.КоличествоКЗаказуДляПоддержанияЗапаса(СтрокаЗапасы);
			СтрокаЗапасы.Отметка = РезультатРасчетаДляОтметки.Отметка И СтрокаЗапасы.КЗаказу > 0;	
		КонецЕсли;
		//-- ШмелевР 16.02.2026 ЗКП.1.2		
	КонецЕсли;
...
Процедура РассчитатьСтрокуТаблицыЗаказыКОбеспечению(ИдентификаторСтроки, Отбор)
	...
	Обработки.ОбеспечениеПотребностейНоваяСхема.ТаблицаПотребностейПоЗаказамИОстатков(
	...
	Если Таблица.Количество() > 0 Тогда
		...
		РезультатРасчета = Обработки.ОбеспечениеПотребностейНоваяСхема.КоличествоКЗаказуДляОбеспеченияЗаказов(СтрокаЗаказы);
...
Процедура ОткрытьМетодОбеспечения(ИмяТаблицы)
    ...
    //++ ШмелевР 16.02.2026 ЗКП.1.2
	ОткрытьФорму("Обработка.ОбеспечениеПотребностей.Форма.ФормаРедактированияПараметровОбеспечения", ПараметрыФормы, ЭтотОбъект, УникальныйИдентификатор);
   //-- ШмелевР 16.02.2026 ЗКП.1.2
КонецПроцедуры
...
Процедура ОткрытьСпособОбеспечения(ИмяТаблицы)
	...
	Если ЗначениеЗаполнено(Строка.СпособОбеспечения) Тогда
		...
		ОткрытьФорму("Справочник.СпособыОбеспеченияПотребностей.Форма.ФормаЭлемента", ПараметрыФормы, ЭтотОбъект, УникальныйИдентификатор);
...
Процедура УстановитьСтатусНаСервере(МассивСсылок, Статус)
	...
	Для Каждого Элемент Из МассивСсылок Цикл
		...
		Исключение
			ТекстОшибки = СтрШаблон(ШаблонОшибкиЗаблокировать, Элемент.Ссылка, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		...
		Исключение
			ТекстОшибки = СтрШаблон(ШаблонОшибкиЗаписать, Элемент.Ссылка, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, Элемент.Ссылка);
		КонецПопытки;
...
Функция ДалееКШагуЗапасы(Результат)
	...
	// НАЧАЛО Османов Н.М. 08.05.2025 
	ТаблицаОсновная = Таблица.Скопировать(, "Номенклатура, Характеристика, ЗаказыКПоступлению");
	ТаблицаОсновная = ТаблицаДоставок(ТаблицаОсновная);
	// КОНЕЦ Османов Н.М. 08.05.2025 
	...
	Для Каждого Товар Из Таблица Цикл
		...
		// НАЧАЛО Османов Н.М. 08.05.2025 
		Если Товар.ЗаказыКПоступлению > 0 Тогда
			ОтборСтрок = Новый Структура("Номенклатура, Характеристика");
			ЗаполнитьЗначенияСвойств(ОтборСтрок, Товар);
			НайденныеСтроки = ТаблицаОсновная.НайтиСтроки(ОтборСтрок);
			Если НайденныеСтроки.Количество() > 0 Тогда
				СтрокаЗапасы.КоличествоДоставка = НайденныеСтроки[0].КоличествоДоставка;
			КонецЕсли;
		КонецЕсли;
		// КОНЕЦ Османов Н.М. 08.05.2025 
		//++ ШмелевР 28.01.2026 ЗКП.2
		// ЗКП.КЗаказу
		Если СтрокаЗапасы.МетодОбеспечения = МетодПоддержанияПоСтатистике Тогда      				
			ДоТочкиЗаказаКПерерасчету = СтрокаЗапасы.Остаток - СтрокаЗапасы.МинимальныйЗапас;
			ВосполнениеСтраховогоЗапаса = Мин(СтрокаЗапасы.СтраховойЗапас, Макс(0, -ДоТочкиЗаказаКПерерасчету));
	...
	// НАЧАЛО Османов Н.М. 08.05.2025 
	ТаблицаНоменклатуры = Объект.Запасы.Выгрузить(, "Номенклатура, КЗаказу");
	ТаблицаНоменклатуры.Свернуть("Номенклатура, КЗаказу");
	ТаблицаНоменклатуры = МинимальныеЕдиницыНоменклатуры(ТаблицаНоменклатуры);
	
	Для Каждого СтрокаЗапасов Из Объект.Запасы Цикл
		Если СтрокаЗапасов.КЗаказу > 0 Тогда
			
			ОтборСтрок = Новый Структура("Номенклатура");
			ЗаполнитьЗначенияСвойств(ОтборСтрок, СтрокаЗапасов);
			НайденныеСтроки = ТаблицаНоменклатуры.НайтиСтроки(ОтборСтрок);
			Если НайденныеСтроки.Количество() > 0 Тогда
				СтрокаЗапасов.КЗаказу = НайденныеСтроки[0].НовоеКоличество;
			КонецЕсли;
...
Процедура УстановитьСтраницыПомощника(ИмяШага)
	...
	ИначеЕсли ИмяШага = "ШагНастройкаСпособовОбеспечения" Тогда
		...
		// НАЧАЛО Османов Н.М. 02.12.2025 
		Элементы.ОткрытьВExcel.Видимость = Ложь;
		// КОНЕЦ Османов Н.М. 02.12.2025 
	...
	ИначеЕсли ИмяШага = "ШагЗапасы" Тогда
		...
		// НАЧАЛО Османов Н.М. 02.12.2025 
		Элементы.ОткрытьВExcel.Видимость = Истина;
		// КОНЕЦ Османов Н.М. 02.12.2025 
...
Процедура КШагуДокументыНаКлиенте()
	...
	Иначе
		...
		ДлительныеОперацииКлиент.ОжидатьЗавершение(ДлительнаяОперация,
			Новый ОписаниеОповещения("ФормированиеЗаказовВФонеЗавершение", ЭтотОбъект),
			ПараметрыОжидания);
...
Процедура ОбработатьДоступностьТиповОбеспечения(ПараметрОтбор)
	...
	ДоступныеТипы = Обработки.ОбеспечениеПотребностейНоваяСхема.ДоступныеТипыОбеспечения();
...
Процедура УстановитьУсловноеОформление()
	...
	// НАЧАЛО Османов Н.М. 27.05.2025 
	Элемент = УсловноеОформление.Элементы.Добавить();

	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ЗапасыОстатокСОбеспечением.Имя);

	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Запасы.ОстатокСОбеспечением");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Меньше;
	ОтборЭлемента.ПравоеЗначение = 0;

	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ПоясняющийОшибкуТекст);
	// КОНЕЦ Османов Н.М. 27.05.2025 
	...
	КонецЕсли;
	#Область УХ_Внедрение
	УстановитьУсловноеОформлениеУХ(ЭтотОбъект);
	#КонецОбласти
КонецПроцедуры
...
Процедура ЗаполнитьИсточникОбеспечения(МассивТиповОбеспечения)
	...
	// &ЗамерПроизводительности
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина,
		"Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.Форма.Команда.ЗаполнитьИсточникОбеспечения");
...
// Османов Н.М. 18.04.2025
&НаСервере
Процедура ОтключитьИспользованиеПоОтбору(НастройкиКомпоновки, НаименованиеОтбора)
	
	МассивЭлементов = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(НастройкиКомпоновки.Отбор, НаименованиеОтбора);
	Для Каждого ЭлементМассива Из МассивЭлементов Цикл
		ЭлементМассива.Использование = Ложь;
	КонецЦикла;
...
// Османов Н.М. 18.04.2025
&НаСервере
Процедура ЗаполнитьТаблицуРасшифровки(СтрокаЗапасы)
	
	СкладыОстатков = Новый Массив;
	НастройкиКомпоновки = КомпоновщикНастроекДляОтборов.ПолучитьНастройки();
	МассивОтборов = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(НастройкиКомпоновки.Отбор, "СкладыОстатков");
	Для Каждого СтрокаОтбора Из МассивОтборов Цикл
		СкладыОстатков = СтрокаОтбора.ПравоеЗначение;
	КонецЦикла;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ИнформацияОДоступности.Заказ КАК ЗаказПоставщику,
		|	//ИнформацияОДоступности.ДатаСобытия КАК ДатаОтгрузки,
		|	ИнформацияОДоступности.ПоступитОстаток КАК Количество,
		|	ТоварыКДоставке.ЗаданиеНаПеревозку КАК ЗаданиеНаПеревозку,
		|	ТоварыКДоставке.Количество КАК КоличествоПоЗаданию,
		|	ТоварыКДоставке.ЗаданиеНаПеревозку.ДатаВремяРейсаФактПо КАК ОкончаниеРейса
		|ИЗ
		...
		|				И Склад В (&СкладыОстатков)
		|				И НЕ Заказ ССЫЛКА Документ.ЗаказНаПроизводство2_2
		|				И НЕ Заказ ССЫЛКА Документ.ПеремещениеТоваров) КАК ИнформацияОДоступности
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТоварыКДоставке КАК ТоварыКДоставке
		|		ПО ИнформацияОДоступности.Заказ = ТоварыКДоставке.Распоряжение
		|			И ИнформацияОДоступности.Номенклатура = ТоварыКДоставке.Номенклатура
		|			И ИнформацияОДоступности.Характеристика = ТоварыКДоставке.Характеристика
		|			И НЕ ТоварыКДоставке.ЗаданиеНаПеревозку.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаданийНаПеревозку.Закрыто)
		|ИТОГИ
		|	МАКСИМУМ(Количество),
		|	СУММА(КоличествоПоЗаданию)
		|ПО
		|	ЗаказПоставщику";
	...
	Запрос.УстановитьПараметр("СкладыОстатков", СкладыОстатков);	
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ДеревоДанных = РезультатЗапроса.Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	ЗначениеВРеквизитФормы(ДеревоДанных, "РасшифровкаЗаказов");
	
	Для Каждого СтрокаЗаказПоставщику Из РасшифровкаЗаказов.ПолучитьЭлементы() Цикл
		
		Для Каждого СтрокаЗаданиеНаПеревозку Из СтрокаЗаказПоставщику.ПолучитьЭлементы() Цикл
			Если Не ЗначениеЗаполнено(СтрокаЗаданиеНаПеревозку.ЗаданиеНаПеревозку) Тогда
				СтрокаЗаказПоставщику.ПолучитьЭлементы().Удалить(СтрокаЗаданиеНаПеревозку);
				Продолжить;
			КонецЕсли;
			СтрокаЗаданиеНаПеревозку.Количество = 0;
			СтрокаЗаданиеНаПеревозку.ЗаказПоставщику = СтрокаЗаданиеНаПеревозку.ЗаданиеНаПеревозку;			
		КонецЦикла;
...
Процедура ВыбратьУпаковкуКлиент(ИмяТаблицы)
	...
	ОткрытьФорму("Обработка.ОбеспечениеПотребностейНоваяСхема.Форма.ФормаВыбораУпаковкиЕдиницы", ПараметрыФормы, ЭтотОбъект, УникальныйИдентификатор);
...
Процедура ИсточникОбеспеченияЗаказыНачалоВыбораУХ(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Поставка.СогласноПланамПоставокПоДоговорам Тогда
		...
		ОткрытьФорму("Обработка.ПодборНоменклатурыПоПлануПоставкиПоДоговору.Форма.ФормаПодбораПоПлануПоставкиПоДоговору", ПараметрыФормы, ЭтотОбъект, УникальныйИдентификатор);
	Иначе
...
Процедура ОбработатьВыборПоПлануПоставки(АдресВХранилище, ТаблицаЗапасы)
	...
	// Типовой код №1
	Если Поставка.Отметка Тогда
		ОтразитьПоставкуСогласноПланамПоставкиВКорзине(ЭтотОбъект, Поставка, 0, НЕ ТаблицаЗапасы);
	КонецЕсли;
	...
	ОтразитьПоставкуСогласноПланамПоставкиВКорзине(ЭтотОбъект, Поставка, КоличествоКЗаказу, НЕ ТаблицаЗапасы);
...
&НаКлиенте
Асинх Процедура РасшифровкаЗаказовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	// НАЧАЛО Османов Н.М. 28.03.2025 
	ТекущаяСтрока = Элементы.РасшифровкаЗаказов.ТекущиеДанные;
	Если ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ИмяПоля = СтрЗаменить(Поле.Имя, "РасшифровкаЗаказов", "");
...
&НаСервере
Функция ТаблицаДоставок(ТаблицаОсновная)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВТ_ОсновнаяТаблица.Номенклатура КАК Номенклатура,
		|	ВТ_ОсновнаяТаблица.Характеристика КАК Характеристика,
		|	ВТ_ОсновнаяТаблица.ЗаказыКПоступлению КАК ЗаказыКПоступлению
		|	
		|ПОМЕСТИТЬ ВТ_ОсновнаяТаблица
		|ИЗ
		|	&ТаблицаОсновная КАК ВТ_ОсновнаяТаблица
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	СУММА(ЕСТЬNULL(ТоварыКДоставке.Количество, 0)) КАК КоличествоДоставка
		|ИЗ
		|	РегистрНакопления.ЗапасыИПотребности.Остатки(
		|			,
		...
		|						Т.Характеристика
		|					ИЗ
		|						ВТ_ОсновнаяТаблица КАК Т
		|					ГДЕ
		|						Т.ЗаказыКПоступлению > 0)
		|				И Склад В (&СкладыОстатков)) КАК ЗапасыИПотребностиОстатки
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТоварыКДоставке КАК ТоварыКДоставке
		|		ПО ЗапасыИПотребностиОстатки.Заказ = ТоварыКДоставке.Распоряжение
		|			И ЗапасыИПотребностиОстатки.Номенклатура = ТоварыКДоставке.Номенклатура
		|			И ЗапасыИПотребностиОстатки.Характеристика = ТоварыКДоставке.Характеристика
		|			И НЕ ТоварыКДоставке.ЗаданиеНаПеревозку.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаданийНаПеревозку.Закрыто)
		|
		|СГРУППИРОВАТЬ ПО
	...
	Запрос.УстановитьПараметр("ТаблицаОсновная", ТаблицаОсновная);
	Запрос.УстановитьПараметр("СкладыОстатков", СкладыОстатков);
...
КонецФункции // ТаблицаДоставок()

&НаСервере
Функция МинимальныеЕдиницыНоменклатуры(ТаблицаНоменклатуры)
	...
	|	И УпаковкиЕдиницыИзмерения.Владелец.ЕдиницаИзмерения = УпаковкиЕдиницыИзмерения.ЕдиницаИзмерения
	|	И УпаковкиЕдиницыИзмерения.Родитель = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|	И НЕ УпаковкиЕдиницыИзмерения.ПометкаУдаления
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_УпаковкиБазовые.Номенклатура КАК Номенклатура,
	|	МИНИМУМ(УпаковкиЕдиницыИзмерения.КоличествоУпаковок) КАК КоличествоУпаковок
	|ПОМЕСТИТЬ ВТ_Коэффициенты
	|ИЗ
	|	ВТ_УпаковкиБазовые КАК ВТ_УпаковкиБазовые
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.УпаковкиЕдиницыИзмерения КАК УпаковкиЕдиницыИзмерения
	|		ПО ВТ_УпаковкиБазовые.ЕдиницаИзмерения = УпаковкиЕдиницыИзмерения.Ссылка.Родитель
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_УпаковкиБазовые.Номенклатура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_Коэффициенты.Номенклатура КАК Номенклатура,
	|	ВЫБОР
	|		КОГДА ЦЕЛ(ВТ_Номенклатура.Количество / ВТ_Коэффициенты.КоличествоУпаковок) = ВТ_Номенклатура.Количество / ВТ_Коэффициенты.КоличествоУпаковок
	|			ТОГДА ВТ_Номенклатура.Количество
	|		ИНАЧЕ ОКР(ВТ_Номенклатура.Количество / ВТ_Коэффициенты.КоличествоУпаковок) * ВТ_Коэффициенты.КоличествоУпаковок
	|	КОНЕЦ КАК НовоеКоличество
	|ИЗ
	|	ВТ_Коэффициенты КАК ВТ_Коэффициенты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Номенклатура КАК ВТ_Номенклатура
	|		ПО ВТ_Коэффициенты.Номенклатура = ВТ_Номенклатура.Номенклатура
	|			И (ВТ_Номенклатура.Количество > 0)
	|			И (ВТ_Коэффициенты.КоличествоУпаковок > 0)";
...
КонецФункции // МинимальныеЕдиницыНоменклатуры()

&НаСервере
Функция СформироватьТабличныйДокументЗапасы()
	
	РезультатФункции = Новый ТабличныйДокумент;
	
	Построитель = Новый ПостроительОтчета;
	Построитель.ИсточникДанных = Новый ОписаниеИсточникаДанных(Объект.Запасы.Выгрузить());
	Построитель.Выполнить();
	Построитель.Вывести(РезультатФункции);
