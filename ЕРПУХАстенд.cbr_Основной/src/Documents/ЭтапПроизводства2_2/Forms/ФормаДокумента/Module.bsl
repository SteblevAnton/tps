...
Процедура cbr_УвеличитьВыпуск(Команда)
	...
	ПараметрыПодбораЭтапов.Вставить("МассивЭтапов",	cbr_МассивЭтапов);
...
Функция cbr_ПолучитьДанныеТЗНаСервере(АдресТЗДанных)
	...
	|ГДЕ
	|	ЭтапПроизводства2_2ПобочныеИзделия.Ссылка В (&МассивЭтапов)
	...
	|ГДЕ
	|	ЭтапПроизводства2_2ВидыРабочихЦентров.Ссылка В (&МассивЭтапов)";
	
	Запрос.УстановитьПараметр("МассивЭтапов", ВыбранныеЭтапы);
	
	МассивРезультатов = Запрос.ВыполнитьПакет();
	
	ВыборкаОбеспечениеМатериаламиИРаботами = МассивРезультатов[0].Выбрать();
	
	Пока ВыборкаОбеспечениеМатериаламиИРаботами.Следующий() Цикл
		
		Отбор = Новый Структура();
		Отбор.Вставить("Номенклатура",ВыборкаОбеспечениеМатериаламиИРаботами.Номенклатура);
		Отбор.Вставить("Характеристика",ВыборкаОбеспечениеМатериаламиИРаботами.Характеристика);
		Отбор.Вставить("Упаковка",ВыборкаОбеспечениеМатериаламиИРаботами.Упаковка);
		Отбор.Вставить("Склад",ВыборкаОбеспечениеМатериаламиИРаботами.Склад);
		Отбор.Вставить("Спецификация",ВыборкаОбеспечениеМатериаламиИРаботами.Спецификация);
		Отбор.Вставить("Подразделение",ВыборкаОбеспечениеМатериаламиИРаботами.Подразделение);
		
		НайденыеСтроки = Объект.ОбеспечениеМатериаламиИРаботами.НайтиСтроки(Отбор);
		
		Если НайденыеСтроки.Количество() Тогда
			Если ВыборкаОбеспечениеМатериаламиИРаботами.Обособленно Тогда
				НоваяСтрока = Объект.ОбеспечениеМатериаламиИРаботами.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаОбеспечениеМатериаламиИРаботами,,"КодСтроки,ИдентификаторСтроки,НомерСтроки");
			Иначе
				НайденыеСтроки[0].Количество = НайденыеСтроки[0].Количество + ВыборкаОбеспечениеМатериаламиИРаботами.Количество;
				НайденыеСтроки[0].КоличествоУпаковок = НайденыеСтроки[0].КоличествоУпаковок + ВыборкаОбеспечениеМатериаламиИРаботами.КоличествоУпаковок;
			КонецЕсли;
		Иначе
			НоваяСтрока = Объект.ОбеспечениеМатериаламиИРаботами.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаОбеспечениеМатериаламиИРаботами,,"КодСтроки,ИдентификаторСтроки,НомерСтроки");
	...
	ВыборкаВыходныеИзделия = МассивРезультатов[1].Выбрать();
	
	Пока ВыборкаВыходныеИзделия.Следующий() Цикл
		
		Отбор = Новый Структура();
		Отбор.Вставить("Номенклатура",ВыборкаВыходныеИзделия.Номенклатура);
		Отбор.Вставить("Характеристика",ВыборкаВыходныеИзделия.Характеристика);
		Отбор.Вставить("Упаковка",ВыборкаВыходныеИзделия.Упаковка);
		Отбор.Вставить("Спецификация",ВыборкаВыходныеИзделия.Спецификация);
		Отбор.Вставить("Подразделение",ВыборкаВыходныеИзделия.Подразделение);
		
		НайденыеСтроки = Объект.ВыходныеИзделия.НайтиСтроки(Отбор);
		
		Если НайденыеСтроки.Количество() Тогда
			НайденыеСтроки[0].Количество = НайденыеСтроки[0].Количество + ВыборкаВыходныеИзделия.Количество;
			НайденыеСтроки[0].КоличествоУпаковок = НайденыеСтроки[0].КоличествоУпаковок + ВыборкаВыходныеИзделия.КоличествоУпаковок;
		Иначе
			НоваяСтрока = Объект.ВыходныеИзделия.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаВыходныеИзделия,,"КодСтроки,ИдентификаторСтроки,НомерСтроки");
		КонецЕсли;
		
	КонецЦикла;
	
	ВыборкаПобочныеИзделия = МассивРезультатов[2].Выбрать();
	
	Пока ВыборкаПобочныеИзделия.Следующий() Цикл
		
		Отбор = Новый Структура();
		Отбор.Вставить("Номенклатура",ВыборкаПобочныеИзделия.Номенклатура);
		Отбор.Вставить("Характеристика",ВыборкаПобочныеИзделия.Характеристика);
		Отбор.Вставить("Упаковка",ВыборкаПобочныеИзделия.Упаковка);
		Отбор.Вставить("Спецификация",ВыборкаПобочныеИзделия.Спецификация);
		Отбор.Вставить("Подразделение",ВыборкаПобочныеИзделия.Подразделение);
		
		НайденыеСтроки = Объект.ПобочныеИзделия.НайтиСтроки(Отбор);
		
		Если НайденыеСтроки.Количество() Тогда
			НайденыеСтроки[0].Количество = НайденыеСтроки[0].Количество + ВыборкаПобочныеИзделия.Количество;
			НайденыеСтроки[0].КоличествоУпаковок = НайденыеСтроки[0].КоличествоУпаковок + ВыборкаПобочныеИзделия.КоличествоУпаковок;
		Иначе
			НоваяСтрока = Объект.ПобочныеИзделия.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаПобочныеИзделия,,"КодСтроки,ИдентификаторСтроки,НомерСтроки");
		КонецЕсли;
		
	КонецЦикла;
	
	ВыборкаВидыРабочихЦентров = МассивРезультатов[3].Выбрать();
	
	Пока ВыборкаВидыРабочихЦентров.Следующий() Цикл
		
		Отбор = Новый Структура();
		Отбор.Вставить("ВидРабочегоЦентра",ВыборкаВидыРабочихЦентров.ВидРабочегоЦентра);
		Отбор.Вставить("ЕдиницаИзмерения",ВыборкаВидыРабочихЦентров.ЕдиницаИзмерения);
		Отбор.Вставить("Использовать",ВыборкаВидыРабочихЦентров.Использовать);
		Отбор.Вставить("ВариантНаладки",ВыборкаВидыРабочихЦентров.ВариантНаладки);
		Отбор.Вставить("Загрузка",ВыборкаВидыРабочихЦентров.Загрузка);
		Отбор.Вставить("ЗагрузкаНорматив",ВыборкаВидыРабочихЦентров.ЗагрузкаНорматив);
		Отбор.Вставить("ВремяРаботыНорматив",ВыборкаВидыРабочихЦентров.ВремяРаботыНорматив);
		
		НайденыеСтроки = Объект.ВидыРабочихЦентров.НайтиСтроки(Отбор);
		
		Если НайденыеСтроки.Количество() Тогда
			НайденыеСтроки[0].ВремяРаботы = НайденыеСтроки[0].ВремяРаботы + ВыборкаВидыРабочихЦентров.ВремяРаботы;
			//НайденыеСтроки[0].ВремяРаботыНорматив = НайденыеСтроки[0].ВремяРаботыНорматив + ВыборкаВидыРабочихЦентров.ВремяРаботыНорматив;
		Иначе
			НоваяСтрока = Объект.ВидыРабочихЦентров.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаВидыРабочихЦентров,,"НомерСтроки");
		КонецЕсли;
