...
Функция cbr_ТабличныйДокументКарточкиНоменклатуры(Номенклатура, НастройкиВидимостиИЗаголовков, ДляПечати) Экспорт
	...
	ПараметрыВыводаРаздела.ИмяОбластиСтрока = "ДополнительныеРеквизитыСтрока";
	ПараметрыВыводаРаздела.ИмяПараметраРеквизит = "ДополнительныеРеквизитыРеквизит";
	ПараметрыВыводаРаздела.ТекстЗначенияРеквизита	= Объект.cbr_ЭтоТендер;
	ПараметрыВыводаРаздела.ТекстЗаголовкаРеквизита	= "Тендер";
	ПараметрыВыводаРаздела.НовоеИмяРаздела = "ЭтоТендер";
	ВывестиРазделСУчетомРазмераЗначений(ТабличныйДокументЛево, Макет, ДляПечати, ПараметрыВыводаРаздела);
	#КонецВставки
	КонецЕсли;
	...
	Если ГлаваВидна("ОбеспечениеИПроизводство", СтруктураВидимости) Тогда
		...
		КонецЕсли;
		#Вставка
		//++ ШмелевР  09.02.2026 ЗКП.1
		Если РазделВиден("cbr_ОптимальнаяПартияЗаказа", СтруктураВидимости) Тогда
			ПараметрыВыводаРаздела = ПараметрыВыводаРазделаСУчетомРазмераЗначений();
			ПараметрыВыводаРаздела.ИмяРаздела = "cbr_ОптимальнаяПартияЗаказа";
			ПараметрыВыводаРаздела.ИмяОбластиСтрока = "cbr_ОптимальнаяПартияЗаказа";
			ПараметрыВыводаРаздела.ИмяПараметраРеквизит = "cbr_ОптимальнаяПартияЗаказа";
			ПараметрыВыводаРаздела.ТекстЗначенияРеквизита =  Объект.cbr_ОптимальнаяПартияЗаказа;
			ВывестиРазделСУчетомРазмераЗначений(ТабличныйДокументЛево, Макет, ДляПечати, ПараметрыВыводаРаздела);
		КонецЕсли;
		//-- ШмелевР  09.02.2026 ЗКП.1
		#КонецВставки
