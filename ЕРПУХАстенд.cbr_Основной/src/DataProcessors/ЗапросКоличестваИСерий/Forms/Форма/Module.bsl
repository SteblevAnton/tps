...
Процедура cbr_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Объект.Регистратор) = Тип("ДокументСсылка.ЗаказКлиента") Тогда 
		...
		ФормыУХ.СоздатьДобавитьРеквизитФормы(ЭтотОбъект, ДобавляемыеРеквизиты, "cbr_ПоказыватьСерииБезУчетаСпецТребований", Новый ОписаниеТипов("Число"));
		ФормыУХ.СоздатьДобавитьРеквизитФормы(ЭтотОбъект, ДобавляемыеРеквизиты, "cbr_ПоказыватьСерииСУчетомСпецТребований", Новый ОписаниеТипов("Булево"));
		ФормыУХ.СоздатьДобавитьРеквизитФормы(ЭтотОбъект, ДобавляемыеРеквизиты, "cbr_НеБолееДатыПроизводства", ОбщегоНазначения.ОписаниеТипаЧисло(3, 0));
		ФормыУХ.СоздатьДобавитьРеквизитФормы(ЭтотОбъект, ДобавляемыеРеквизиты, "cbr_ДопустимыйОстатокГСХНеМенееПроцент", ОбщегоНазначения.ОписаниеТипаЧисло(3, 0));
		ФормыУХ.СоздатьДобавитьРеквизитФормы(ЭтотОбъект, ДобавляемыеРеквизиты, "cbr_ДопустимыйОстатокГСХНеМенееМес", ОбщегоНазначения.ОписаниеТипаЧисло(3, 0));
		ЭтотОбъект.ИзменитьРеквизиты(ДобавляемыеРеквизиты);
		
		НовыйЭлемент = ФормыУХ.СоздатьПолеФормы(ЭтотОбъект.Элементы, "cbr_ПоказыватьСерииБезУчетаСпецТребований",, 
		"cbr_ПоказыватьСерииБезУчетаСпецТребований", ВидПоляФормы.ПолеПереключателя,
        ЭтотОбъект.Элементы.КоманднаяПанельСерии, ЭтотОбъект.Элементы.КоманднаяПанельСерииСлева);
		НовыйЭлемент.УстановитьДействие("ПриИзменении", "cbr_ПоказыватьСерииБезУчетаСпецТребованийПриИзменении");
