...
Процедура cbr_РасставитьПродукциюПоПаллетамНаСервере(ВыбранныеСтроки)
	...
	ОставленныеСтрокиТЧ.Колонки.Добавить("ЭтоПаллет",Новый ОписаниеТипов("Булево"));
	...
	Для Каждого ИндексСтроки Из ВыбранныеСтроки Цикл
		...
		Иначе
			...
			Если СтруктураНабора.ЕдиницаИзмерения = ЗначениеПаллетов Тогда
				ОставленнаяСтрока.ЭтоПаллет = Истина;	
			КонецЕсли;
			ТаблицаДокумента[ИндексСтроки].Используется = Истина;
		КонецЕсли;
	...
	Пока ИтоговаяТаблица.Количество() > 0 Цикл
		...
		Если СменаУровня Тогда
			...
			Если Уровень > 1 Тогда
				ОстатокПлощадиПаллетаЕвро = ОстатокПлощадиПаллетаЕвро - ПлощадьПервогоУровня;
			КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из ИтоговаяТаблица Цикл
			...
			Если СтрокаТаблицы.cbr_Уровень = 0 И СтрокаТаблицы.cbr_НомерПаллета = 0
				...
				Иначе
					...
					СтрокаОтложенной = ТаблицаОтложенных.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаОтложенной, СтрокаТаблицы);
					//ДокладываемПервыйУровень = Истина;
			...
			ИначеЕсли СтрокаТаблицы.cbr_Уровень = 0 И СтрокаТаблицы.cbr_НомерПаллета = 0 И ПлощадьУпаковки
				<= ОстатокПлощадиПаллетаЕвро И ВесУпаковки <= ОстатокВесаПаллетаЕвро И ВысотаУпаковки <= ОстатокВысотыПаллетаЕвро Тогда
					
				Если МаксимальнаяВысотаУровня < ВысотаУпаковки Тогда
					МаксимальнаяВысотаУровня = СтрокаТаблицы.Высота;
				КонецЕсли;	

				СтрокаТаблицы.КоличествоУпаковок = Мин(КоличествоНачальное ,Цел(ОстатокПлощадиПаллетаЕвро / ПлощадьУпаковки));
				...
				Если ПерваяПродукцияУровня.Получить(Строка(МаксимальныйНомерПаллета) + "." + Строка(Уровень)) = Неопределено Тогда
				...
				Если КоличествоНачальное > СтрокаТаблицы.КоличествоУпаковок Тогда
					...
					СтрокаТаблицы.КоличествоУпаковок = КоличествоОсталось;
					СтрокаТаблицы.cbr_НомерПаллета = 0;
					СтрокаТаблицы.cbr_Уровень = 0;
				...
				КонецЕсли;
			ИначеЕсли (СтрокаТаблицы.cbr_Уровень = 0 И СтрокаТаблицы.cbr_НомерПаллета = 0) И (ПлощадьУпаковки
				> ОстатокПлощадиПаллетаЕвро Или ВесУпаковки > ОстатокВесаПаллетаЕвро ИЛИ ВысотаУпаковки > ОстатокВысотыПаллетаЕвро) Тогда
				...
				Если ПлощадьУпаковки > ОстатокПлощадиПаллетаЕвро И ОстатокПлощадиПаллетаЕвро
					...
					СтрокаОставленная = ОставленныеСтрокиТЧ.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаОставленная, СтрокаТаблицы);
					СтрокаОставленная.Используется = Истина;
				...
				ИначеЕсли ВесУпаковки > ОстатокВесаПаллетаЕвро И ОстатокВесаПаллетаЕвро = РеквизитыПаллетаЕвро.Вес Тогда
					...
					"КоличествоУпаковок, Количество");
					Прервать;
				ИначеЕсли ВысотаУпаковки > ОстатокВысотыПаллетаЕвро И ОстатокВысотыПаллетаЕвро = РеквизитыПаллетаЕвро.Высота Тогда
					СтрокаОставленная = ОставленныеСтрокиТЧ.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаОставленная, СтрокаТаблицы);
					СтрокаОставленная.Используется = Истина;
					ИтоговаяТаблица.Удалить(ИндексСтроки);
	...
	ТолькоПаллеты = Истина;
	СписокПозицийНеРаставленных = "";
	...
	Для Каждого ТоварОставшийся Из ОставленныеСтрокиТЧ Цикл
		...
		Если НЕ ТоварОставшийся.ЭтоПаллет И ТоварОставшийся.Используется Тогда
			СписокПозицийНеРаставленных = СписокПозицийНеРаставленных + Символы.ПС + ТоварОставшийся.Номенклатура + " " + ТоварОставшийся.Упаковка + " в количестве " + ТоварОставшийся.КоличествоУпаковок + " шт";
			ТолькоПаллеты = Ложь;	
		КонецЕсли
	КонецЦикла;
	...
	Если ТолькоПаллеты Тогда
	
		ОбщегоНазначения.СообщитьПользователю("Вся продукция расставлена по паллетам.");
	
	Иначе
		
		СообщениюОСписке = СтрШаблон("Продукция расставлена по паллетам не полностью. Не удалось расставить на паллеты позиции: %1." +Символы.ПС +"Проставьте номера паллет и уровни вручную.", СписокПозицийНеРаставленных);
		ОбщегоНазначения.СообщитьПользователю(СообщениюОСписке);
	КонецЕсли;
