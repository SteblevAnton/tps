		...
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТовары.Номенклатура, "ТипНоменклатуры") =
			...
			Если НЕ ЗначениеЗаполнено(СтрокаТовары.Спецификация) Тогда 
				Если РаботаемПоСоглашению И НЕ СтрокаТовары.cbr_ЭтоПФ Тогда
					...
					Продолжить;												
				КонецЕсли;				
				cbr_РаботаСОшибкойАктуальности(Неопределено, Дата, СтрокаТовары.Номенклатура);		
			КонецЕсли;
		Иначе
			СтрокаТовары.cbr_Актуальность = Истина;
		КонецЕсли;					
...
Процедура cbr_РаботаСОшибкойАктуальности(Спецификация, Дата, Номенклатура)
	...
	Если Спецификация = Неопределено Тогда
		СообщениеОбОшибке = "По номенклатуре " + Строка(Номенклатура) + " не найдено спецификаций!";
	...
	Иначе
		СообщениеОбОшибке = "По спецфикации " + Строка(Спецификация) + " не пройдена проверка актуальности!";
		ОбщегоНазначения.СообщитьПользователю(СообщениеОбОшибке);
		СобытиеЖурналаРегистрации = "ЗаказКлиента.ПередЗаписью";
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации, УровеньЖурналаРегистрации.Предупреждение, , , СообщениеОбОшибке);		
	КонецЕсли;
