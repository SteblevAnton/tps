...
Процедура cbr_ЗаполнитьРасходы(ДокументОснование, НесколькоОрганизаций) Экспорт
    ...
    Подразделение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Справочники.cbr_ПредопределенныеЗначенияКонстант.ТПСОтделЛогистикиИВЭД, "Значение");
    СтавкаНДС = СтавкаНДС();
    ...
    ТранпортныеУслуги = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Справочники.cbr_ПредопределенныеЗначенияКонстант.ТранспортныеУслугиДоставкаПокупателюТранспортнойКомпании, "Значение");
    ТранпортныеУслуги44 = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Справочники.cbr_ПредопределенныеЗначенияКонстант.ТранспортныеУслугиДоставкаПокупателюТранспортнойКомпании44, "Значение");
    ТранпортныеУслуги26 = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Справочники.cbr_ПредопределенныеЗначенияКонстант.ТранспортныеУслугиДоставкаПокупателюТранспортнойКомпании26, "Значение");
    ...
    Для Каждого Строка Из ТЗ Цикл
        ...
        Если ТипЗнч(Строка.Распоряжение) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
            Если Строка.Организация = Справочники.Организации.НайтиПоРеквизиту("ИНН", "5042063026") Тогда
                НоваяСтрока.СтатьяРасходов = ТранпортныеУслуги26;
            Иначе
                НоваяСтрока.СтатьяРасходов = ТранпортныеУслуги44;
            КонецЕсли;
        ИначеЕсли ТипЗнч(Строка.Распоряжение) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
            НоваяСтрока.СтатьяРасходов = ТранпортныеУслуги;
        КонецЕсли;
