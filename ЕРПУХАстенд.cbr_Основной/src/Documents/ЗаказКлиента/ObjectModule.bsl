...
Процедура cbr_ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		cbr_МаксимальнаяРучнаяСкидка = 0;
		Для Каждого СтрТЧ Из Товары Цикл
			Если Не СтрТЧ.cbr_ПоСоглашению Тогда
				cbr_МаксимальнаяРучнаяСкидка = Макс(cbr_МаксимальнаяРучнаяСкидка, СтрТЧ.ПроцентРучнойСкидки);
			КонецЕсли;
		КонецЦикла;
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ЗаказКлиентаТовары.Ссылка.cbr_СтоимостьДоставкиСНДС / ЗаказКлиентаТовары.Ссылка.СуммаДокумента * 100 КАК ПроцентТУ
		|ИЗ
		|	Документ.ЗаказКлиента.Товары КАК ЗаказКлиентаТовары
		|ГДЕ
		|	ЗаказКлиентаТовары.Ссылка = &Ссылка
		...
		Пока Выборка.Следующий() Цикл
			cbr_ПроцентТУ = Выборка.ПроцентТУ;
		КонецЦикла;

		cbr_МаксимальнаяРучная = cbr_МаксимальнаяРучнаяСкидка + cbr_ПроцентТУ + 1;

		cbr_ПроцентАВ = 1;
		Для Каждого Стр Из cbr_АгентскиеВознаграждения Цикл
			cbr_ПроцентАВ = cbr_ПроцентАВ + Стр.СтавкаАВ;
		КонецЦикла;
	...
	СуммаПроцентАВЗаказа = cbr_АгентскиеВознаграждения.Итог("СтавкаАВ");

	Если СуммаПроцентАВСоглашения = СуммаПроцентАВЗаказа Тогда
		cbr_СовпадениеСуммПроцентаАВ = Истина;
		cbr_ПроцентАВ = 1;
	Иначе
		cbr_СовпадениеСуммПроцентаАВ = Ложь;
	КонецЕсли;
