...
Процедура cbr_ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если Этап.Спецификация.ОсновноеИзделиеЭтап.лимсОперативныйКонтроль = истина
		И Этап.Спецификация.ОсновноеИзделиеЭтап.ЛимсВидыПроверки[0].ВидПроверки = "Выходной контроль"
		И Статус.Наименование = Перечисления.СтатусыЭтаповПроизводства2_2.Начат Тогда
		
		ПризнакПроизведено = Истина;
		Для каждого Строка из ВыходныеИзделия Цикл
			Если Строка.Произведено = Истина тогда
				Продолжить;
			Иначе
				ПризнакПроизведено = ложь;
			КонецЕсли;				
		КонецЦикла;		 
		
		Если ПризнакПроизведено Тогда		
			Запрос = новый Запрос;
			...
			|ГДЕ
			|	лимсЗаявкаНаКонтроль.ПометкаУдаления = ЛОЖЬ
			|	И лимсЗаявкаНаКонтроль.Проведен = ИСТИНА
			...
			Если РезультатЗапрос.Следующий() Тогда
				ОбщегоНазначения.СообщитьПользователю("По данному этапу производства уже создана заявка на контроль:" + РезультатЗапрос.Ссылка);
			Иначе
				Попытка
					ЗаявкаНаКонтрольОбъект = Документы.лимсЗаявкаНаКонтроль.СоздатьДокумент();
					ЗаявкаНаКонтрольОбъект.Заполнить(Ссылка);
					ЗаявкаНаКонтрольОбъект.Записать(РежимЗаписиДокумента.Проведение);
				Исключение
					ОбщегоНазначения.СообщитьПользователю("Заявка на контроль не создана!");
					ЗаписьЖурналаРегистрации("Заявка на контроль не создана!",УровеньЖурналаРегистрации.Предупреждение,,,,);
				КонецПопытки;									
			КонецЕсли;		
		КонецЕсли;
