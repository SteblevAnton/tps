...
Процедура УстановитьСвойстваДинамическогоСписка()
	...
	|ПОМЕСТИТЬ втДопустимаяНоменклатура
	|ИЗ
	...
	|		ПО (ДвижениеПродукцииИМатериаловТовары.Распоряжение = ЭтапПроизводства2_2.Ссылка)
	|			И (ЭтапПроизводства2_2.Распоряжение <> ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство2_2.ПустаяСсылка))
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	втДопустимаяНоменклатура.ЗаказНаПроизводство КАК ЗаказНаПроизводство,
	|	втДопустимаяНоменклатура.Номенклатура КАК Номенклатура,
	|	втДопустимаяНоменклатура.Характеристика КАК Характеристика,
	|	втДопустимаяНоменклатура.Серия КАК Серия
	|ПОМЕСТИТЬ втОтобраннаяНоменклатура
	|ИЗ
	|	втДопустимаяНоменклатура КАК втДопустимаяНоменклатура
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказНаПроизводство2_2.Продукция КАК ЗаказНаПроизводство2_2Продукция
	|		ПО втДопустимаяНоменклатура.ЗаказНаПроизводство = ЗаказНаПроизводство2_2Продукция.Ссылка
	|			И втДопустимаяНоменклатура.Номенклатура = ЗаказНаПроизводство2_2Продукция.Номенклатура
	|			И втДопустимаяНоменклатура.Характеристика = ЗаказНаПроизводство2_2Продукция.Характеристика
	|
	|СГРУППИРОВАТЬ ПО
	|	втДопустимаяНоменклатура.ЗаказНаПроизводство,
	|	втДопустимаяНоменклатура.Номенклатура,
	|	втДопустимаяНоменклатура.Характеристика,
	|	втДопустимаяНоменклатура.Серия
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	лимсИсторияСтатусаСерииСрезПоследних.Номенклатура КАК Номенклатура,
	|	лимсИсторияСтатусаСерииСрезПоследних.Характеристика КАК Характеристика,
	|	лимсИсторияСтатусаСерииСрезПоследних.Серия КАК Серия,
	|	лимсИсторияСтатусаСерииСрезПоследних.ВидПроверки КАК ВидПроверки,
	|	лимсИсторияСтатусаСерииСрезПоследних.СтатусСерии КАК СтатусСерии
	|ПОМЕСТИТЬ втИсторияДвжиенияНоменклатуры
	|ИЗ
	|	РегистрСведений.лимсИсторияСтатусаСерии.СрезПоследних(
	|			,
	...
	|					ИЗ
	|						втДопустимаяНоменклатура КАК втДопустимаяНоменклатура)
	|				И ВидПроверки = &ВидПроверки) КАК лимсИсторияСтатусаСерииСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	втДопустимаяНоменклатура.Номенклатура КАК Номенклатура,
	|	втДопустимаяНоменклатура.Характеристика КАК Характеристика,
	|	МАКСИМУМ(втИсторияДвжиенияНоменклатуры.СтатусСерии = &Брак) КАК БракНаСкладе
	|ПОМЕСТИТЬ ДанныеПоИстрииСтатуса
	|ИЗ
	|	втДопустимаяНоменклатура КАК втДопустимаяНоменклатура
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втИсторияДвжиенияНоменклатуры КАК втИсторияДвжиенияНоменклатуры
	|		ПО втДопустимаяНоменклатура.Номенклатура = втИсторияДвжиенияНоменклатуры.Номенклатура
	|			И втДопустимаяНоменклатура.Характеристика = втИсторияДвжиенияНоменклатуры.Характеристика
	|			И втДопустимаяНоменклатура.Серия = втИсторияДвжиенияНоменклатуры.Серия
	|
	|СГРУППИРОВАТЬ ПО
	|	втДопустимаяНоменклатура.ЗаказНаПроизводство,
	|	втДопустимаяНоменклатура.Номенклатура,
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ втДопустимаяНоменклатура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ втИсторияДвжиенияНоменклатуры
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ втОтобраннаяНоменклатура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	МАКСИМУМ(ЕСТЬNULL(ДанныеПоИстрииСтатуса.БракНаСкладе, ЛОЖЬ)) КАК БракНаСкладе,
	|	СТРОКА(ЗаказНаПроизводство2_2Продукция.НомерСтроки) КАК НомерСтроки
	|ПОМЕСТИТЬ ВТ_Продукция
	|ИЗ
	|	Документ.ЗаказНаПроизводство2_2 КАК ВТ_Таблица
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказНаПроизводство2_2.Продукция КАК ЗаказНаПроизводство2_2Продукция
	|			ЛЕВОЕ СОЕДИНЕНИЕ ДанныеПоИстрииСтатуса КАК ДанныеПоИстрииСтатуса
	|			ПО (ЗаказНаПроизводство2_2Продукция.Ссылка = ДанныеПоИстрииСтатуса.ЗаказНаПроизводство)
	|				И (ЗаказНаПроизводство2_2Продукция.Номенклатура = ДанныеПоИстрииСтатуса.Номенклатура)
	|				
	...
	|	СТРОКА(ЗаказНаПроизводство2_2Продукция.НомерСтроки)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА ЭтапПроизводства.Ссылка ЕСТЬ NULL
	|				ТОГДА ЛОЖЬ
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ) КАК НаличиеЭтапов,
	|	ЕСТЬNULL(ЭтапПроизводства.cbr_Брак, 0) > 0 КАК Брак
	...
	|СГРУППИРОВАТЬ ПО
	|	Таблица.Ссылка,
	|	ЕСТЬNULL(ЭтапПроизводства.cbr_Брак, 0) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА ВТ_Продукция.ДокументОснование ССЫЛКА Документ.ЗаказКлиента
	|			ТОГДА ВЫРАЗИТЬ(ВТ_Продукция.ДокументОснование КАК Документ.ЗаказКлиента).Контрагент
	...
	|	ВЫБОР
	|		КОГДА ВТ_Продукция.КоличествоУпаковок - ЕСТЬNULL(ВТ_ЭтапПроизводстваИтоги.Выполнено, 0) - ЕСТЬNULL(ВТ_ЭтапПроизводстваИтоги.СвободныйОстаток, 0) > 0
	|			ТОГДА ВТ_Продукция.КоличествоУпаковок - ЕСТЬNULL(ВТ_ЭтапПроизводстваИтоги.Выполнено, 0) - ЕСТЬNULL(ВТ_ЭтапПроизводстваИтоги.СвободныйОстаток, 0)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ОстатокКИсполнению,
	|	ЕСТЬNULL(ВТ_ЭтапПроизводстваНаличие.НаличиеЭтапов, ЛОЖЬ) КАК НаличиеЭтапов,
	|	ЕСТЬNULL(ВТ_ЭтапПроизводстваНаличие.Брак, ЛОЖЬ) КАК Брак,
	|	ЕСТЬNULL(ВТ_Продукция.БракНаСкладе, ЛОЖЬ) КАК БракНаСкладе,
	...
	БракФакт = ПредопределенноеЗначение("Справочник.cbr_ПредопределенныеЗначенияКонстант.ОчередьАРМ_БракНаСкладе");
	ВидПроверкиСвязи = ПредопределенноеЗначение("Справочник.cbr_ПредопределенныеЗначенияКонстант.ОчередьАРМ_ВидПроверки");
	...
	БракФактЗначение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(БракФакт, "Значение");
	ВидПроверкиСвязиЗначение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидПроверкиСвязи, "Значение");
	...
	Список.Параметры.УстановитьЗначениеПараметра("Брак", БракФактЗначение);
	Список.Параметры.УстановитьЗначениеПараметра("ВидПроверки", ВидПроверкиСвязиЗначение);
