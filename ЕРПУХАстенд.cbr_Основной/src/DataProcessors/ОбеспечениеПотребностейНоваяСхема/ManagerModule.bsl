...
Процедура ТаблицаЗапасы(Параметры, АдресРезультата) Экспорт
	...
	Если  Параметры.ТипОтбора = "ОтборПоЗначениямСФормы" Тогда	//++ ШмелевР 16.02.2026 ЗКП.1.2
	...
	Текст =
		...
		|	&НачалоИсторииПотребления КАК НачалоИсторииПотребления,
		|	&ОкончаниеИсторииПотребления КАК ОкончаниеИсторииПотребления
		|ИЗ
		|	ВтТовары КАК Т
		|
