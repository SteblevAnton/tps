...
Процедура cbr_ЗаполнитьДокументНаОснованииЗаказаКлиента(Знач ДокументОснование, Знач СкладОтгрузки, ВариантОформления, РеквизитыЗаказа, ПараметрыОформления, ХозяйственнаяОперацияРеализации)
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
		...
		|ИЗ
		|	РегистрНакопления.РаспоряженияНаОтгрузкуИВозврат.Обороты(,,, Распоряжение = &ДокументОснование) КАК РаспоряженияОбороты";
	...
	ИначеЕсли ТипОснования = Тип("Массив") Тогда
		...
		Если РеквизитыЗаказа.ЗапрещеноВыбиратьГруппуСкладов Тогда
			...
			|	РаспоряженияОбороты.Склад КАК Склад
			|ИЗ
			|	РегистрНакопления.РаспоряженияНаОтгрузкуИВозврат.Обороты(,,, Распоряжение В (&МассивДокументов)) КАК РаспоряженияОбороты";
	...
	//Заполнение ТЧ cbr_ТЧПересечениеМенеджеров
	Если ТипЗнч(ДокументОснование) = Тип("Массив") Тогда
		Для Каждого Стр Из ДокументОснование[0].cbr_ТЧПересечениеМенеджеров Цикл
			НовыйСтр = cbr_ТЧПересечениеМенеджеров.Добавить();
			ЗаполнитьЗначенияСвойств(НовыйСтр, Стр);
		КонецЦикла; 
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		Для Каждого Стр Из ДокументОснование.cbr_ТЧПересечениеМенеджеров Цикл
			НовыйСтр = cbr_ТЧПересечениеМенеджеров.Добавить();
			ЗаполнитьЗначенияСвойств(НовыйСтр, Стр);
		КонецЦикла;	 
	КонецЕсли;
