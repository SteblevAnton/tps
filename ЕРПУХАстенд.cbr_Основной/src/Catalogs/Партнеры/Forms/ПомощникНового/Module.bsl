...
Процедура cbr_ОбработкаВыбораОтраслиКонтрагента(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если Не ЗначениеЗаполнено(РезультатЗакрытия) Тогда
		Возврат;
	КонецЕсли;
	Контейнер = Элементы.cbr_ОтрасльКонтрагента;
...
Функция cbr_ДополнительныеСведенияЗаполненыНеправильно()
	...
	#Вставка
	//++ AlopinAS 13.02.2026 
	Если ЭтотОбъект.Клиент и Не ЗначениеЗаполнено(ЭтотОбъект.cbr_ОтрасльКонтрагента) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Обязательно должен быть заполен реквезит отрасль контрагента'"));
		Отказ = Истина;
	КонецЕсли;
	//-- AlopinAS 13.02.2026 
	#КонецВставки	
