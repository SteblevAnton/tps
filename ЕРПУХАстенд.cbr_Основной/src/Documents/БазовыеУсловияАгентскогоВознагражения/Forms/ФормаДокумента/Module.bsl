...
&НаКлиенте
Процедура ПартнерАгентаПриИзменении(Элемент)
	
	ПартнерыИКонтрагентыВызовСервера.ЗаполнитьКонтрагентаПартнераПоУмолчанию(Объект.ПартнерАгента, Объект.Агент, Истина);
...
Процедура АгентПриИзменении(Элемент)
	...
	Объект.ПартнерАгента = ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъекта(Объект.Агент, "Партнер");
...
&НаКлиенте
Процедура КлиентыПартнерКлиентаПриИзменении(Элемент)
	...
	ПартнерыИКонтрагентыВызовСервера.ЗаполнитьКонтрагентаПартнераПоУмолчанию(ТекущиеДанные.ПартнерКлиента, ТекущиеДанные.Клиент, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура КлиентыДатаРеализацииПриИзменении(Элемент)
	...
	Если ТекущиеДанные.ДатаРеализации < ТекущаяДата() Тогда
		ТекстСообщения = НСтр("ru='Указанная дата меньше текущей даты. Укажите другую дату!'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		ТекущиеДанные.ДатаРеализации = Дата(1, 1, 1);
	КонецЕсли;
