...
&ИзменениеИКонтроль("НоменклатурныйПланВМенеджерВременныхТаблиц")
Процедура cbr_НоменклатурныйПланВМенеджерВременныхТаблиц(МенеджерВременныхТаблиц, НастройкиДляОтборов, ПользовательскиеОтборы)
	...
	Если СтрНайти(Запрос.Текст, "ПОМЕСТИТЬ НоменклатурныйПлан") <> 0 Тогда
...
&ИзменениеИКонтроль("ПозицииПланаКЗапускуВПроизводствоВнутриЗамераВремени")
Функция cbr_ПозицииПланаКЗапускуВПроизводствоВнутриЗамераВремени(Параметры, АдресХранилища)
	...
	ТекстЗапроса =
		...
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	План.ЗаказНаПроизводствоДата                    КАК ЗаказНаПроизводствоДата,
		|	План.ВидСтроки                    				КАК ВидСтроки,
		|	План.Спецификация                               КАК Спецификация,
		|	План.Организация                                КАК Организация,
		|	План.Партнер                                    КАК Партнер,
		...
		|	Группы.Спецификация                             КАК Спецификация,
		|	Группы.Организация                              КАК Организация,
		|	Группы.ЗаказНаПроизводствоДата                  КАК ЗаказНаПроизводствоДата,
		|	Группы.ВидСтроки                                КАК ВидСтроки,
		|	Группы.Партнер                                  КАК Партнер,
		|	Группы.Договор                                  КАК Договор,
		|	Группы.НаправлениеДеятельности                  КАК НаправлениеДеятельности,
		|	Группы.ПодразделениеДиспетчер                   КАК ПодразделениеДиспетчер,
		...
		|		И План.ЗаказПодДеятельность = Группы.ЗаказПодДеятельность
		|		И План.ХозяйственнаяОперация = Группы.ХозяйственнаяОперация
		|		И План.ЗаказНаПроизводствоДата = Группы.ЗаказНаПроизводствоДата
		|		И План.ВидСтроки = Группы.ВидСтроки
		|		И (НЕ Группы.ГруппировкаПоНоменклатуре
		|			ИЛИ План.Номенклатура = Группы.НоменклатураГруппировка
		|				И План.Характеристика = Группы.ХарактеристикаГруппировка)
		|
		...
		|
		|УПОРЯДОЧИТЬ ПО
		|	План.ЗаказНаПроизводствоДата УБЫВ,
		|	План.ВидСтроки УБЫВ,
		|	Группы.НомерГруппы,
		|	План.Номенклатура,
		|	План.Характеристика
