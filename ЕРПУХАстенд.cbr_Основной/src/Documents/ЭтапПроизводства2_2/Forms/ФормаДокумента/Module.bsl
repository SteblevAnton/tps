...
Процедура cbr_ПриОткрытииПосле(Отказ)
	cbr_ОбновитьВремяКонтроля();	
КонецПроцедуры //-- Мохов ПРЗ.18

//++ Мохов ПРЗ.18
&НаКлиенте
Процедура cbr_ОбновитьВремяКонтроля()
	Если Объект.cbr_ДатаНачалоКонтроля <> Объект.cbr_ДатаОкончаниеКонтроля Тогда
		cbr_ВремяКонтроляДата = cbr_СекундыВВремя(Объект.cbr_ВремяКонтроля + 
			Число(ОбщегоНазначенияКлиент.ДатаСеанса() - Объект.cbr_ДатаНачалоКонтроля));		
...
//++ Мохов ПРЗ.18
&НаКлиенте
Процедура cbr_СтатусКонтроляПриИзменении()
	cbr_ОбновитьВремяКонтроля();	
КонецПроцедуры //-- Мохов ПРЗ.18	
...
КонецФункции //-- Мохов ПРЗ.18

&НаКлиенте
Процедура cbr_ПослеЗаписиПосле(ПараметрыЗаписи)
	//++ Мохов ПРЗ.18
	cbr_ОбновитьВремяКонтроля();
	//-- Мохов ПРЗ.18
КонецПроцедуры
...
&НаСервере
Процедура cbr_ОбработатьИзменениеСтатусаНаСервере()
...
Процедура cbr_СверткаТЗВыходныеИзделия()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТЧГруппировка.Номенклатура КАК Номенклатура,
		|	ТЧГруппировка.Характеристика КАК Характеристика,
		|	ТЧГруппировка.Упаковка КАК Упаковка,
		|	ТЧГруппировка.Получатель КАК Получатель,
		|	ТЧГруппировка.Назначение КАК Назначение,
		|	ТЧГруппировка.ДатаПроизводства КАК ДатаПроизводства,
		|	ТЧГруппировка.СтатусУказанияСерий КАК СтатусУказанияСерий,
		|	ТЧГруппировка.СтатусУказанияСерийОтправитель КАК СтатусУказанияСерийОтправитель,
		|	ТЧГруппировка.СтатусУказанияСерийПолучатель КАК СтатусУказанияСерийПолучатель,
		|	ТЧГруппировка.ДоляСтоимости КАК ДоляСтоимости,
		|	ТЧГруппировка.ДоляСтоимостиНаЕдиницу КАК ДоляСтоимостиНаЕдиницу,
		|	ТЧГруппировка.СписатьНаРасходы КАК СписатьНаРасходы,
		|	ТЧГруппировка.СтатьяРасходов КАК СтатьяРасходов,
		|	ТЧГруппировка.АналитикаРасходов КАК АналитикаРасходов,
		|	ТЧГруппировка.АналитикаАктивовПассивов КАК АналитикаАктивовПассивов,
		|	ТЧГруппировка.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
		|	ТЧГруппировка.ОформитьОтчетДавальцу КАК ОформитьОтчетДавальцу,
		|	ТЧГруппировка.ОбработатьПоСпецификации КАК ОбработатьПоСпецификации,
		|	ТЧГруппировка.Спецификация КАК Спецификация,
		|	ТЧГруппировка.ЭтапПотребитель КАК ЭтапПотребитель,
		|	ТЧГруппировка.Период КАК Период,
		|	ТЧГруппировка.Подразделение КАК Подразделение,
		|	ТЧГруппировка.НастройкаСчетовУчета КАК НастройкаСчетовУчета,
		|	ТЧГруппировка.ИспользуетсяАвтовыбор КАК ИспользуетсяАвтовыбор,
		|	ТЧГруппировка.ХарактеристикиИспользуются КАК ХарактеристикиИспользуются,
		|	ТЧГруппировка.ТипНоменклатуры КАК ТипНоменклатуры,
		|	ТЧГруппировка.НаправлениеВыпуска КАК НаправлениеВыпуска,
		|	ТЧГруппировка.ЦеховаяКладовая КАК ЦеховаяКладовая,
		|	ТЧГруппировка.ТипСтатьи КАК ТипСтатьи,
		|	ТЧГруппировка.АналитикаРасходовОбязательна КАК АналитикаРасходовОбязательна,
		|	ТЧГруппировка.АналитикаРасходовЗаказРеализация КАК АналитикаРасходовЗаказРеализация,
		|	ТЧГруппировка.НазначениеРедактирование КАК НазначениеРедактирование,
		|	ТЧГруппировка.ДоляСтоимостиОбязательна КАК ДоляСтоимостиОбязательна,
		|	ТЧГруппировка.ЕстьОшибкиЗаполненияДолиСтоимости КАК ЕстьОшибкиЗаполненияДолиСтоимости,
		|	ТЧГруппировка.АналитикаАктивовПассивовНеИспользуется КАК АналитикаАктивовПассивовНеИспользуется,
		|	ТЧГруппировка.НужноОкруглять КАК НужноОкруглять,
		|	ТЧГруппировка.ДоступноОформлениеОтчетаДавальцу КАК ДоступноОформлениеОтчетаДавальцу,
		|	ТЧГруппировка.КоличествоУпаковок КАК КоличествоУпаковок,
		|	ТЧГруппировка.Количество КАК Количество,
		|	ТЧГруппировка.Цена КАК Цена,
		|	ТЧГруппировка.Сумма КАК Сумма,
		|	ТЧГруппировка.ДоляСтоимостиПроцент КАК ДоляСтоимостиПроцент
		...
		|	втГруппировка.Упаковка КАК Упаковка,
		|	втГруппировка.Получатель КАК Получатель,
		|	втГруппировка.Назначение КАК Назначение,
		|	втГруппировка.ДатаПроизводства КАК ДатаПроизводства,
		|	МАКСИМУМ(втГруппировка.СтатусУказанияСерий) КАК СтатусУказанияСерий,
		|	МАКСИМУМ(втГруппировка.СтатусУказанияСерийОтправитель) КАК СтатусУказанияСерийОтправитель,
		|	МАКСИМУМ(втГруппировка.СтатусУказанияСерийПолучатель) КАК СтатусУказанияСерийПолучатель,
		|	СУММА(втГруппировка.ДоляСтоимости) КАК ДоляСтоимости,
		|	СУММА(втГруппировка.ДоляСтоимостиНаЕдиницу) КАК ДоляСтоимостиНаЕдиницу,
		|	втГруппировка.СписатьНаРасходы КАК СписатьНаРасходы,
		|	втГруппировка.СтатьяРасходов КАК СтатьяРасходов,
		|	втГруппировка.АналитикаРасходов КАК АналитикаРасходов,
		|	втГруппировка.АналитикаАктивовПассивов КАК АналитикаАктивовПассивов,
		|	втГруппировка.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
		|	втГруппировка.ОформитьОтчетДавальцу КАК ОформитьОтчетДавальцу,
		|	втГруппировка.ОбработатьПоСпецификации КАК ОбработатьПоСпецификации,
		|	втГруппировка.Спецификация КАК Спецификация,
		|	втГруппировка.ЭтапПотребитель КАК ЭтапПотребитель,
		|	втГруппировка.Период КАК Период,
		|	втГруппировка.Подразделение КАК Подразделение,
		|	втГруппировка.НастройкаСчетовУчета КАК НастройкаСчетовУчета,
		|	втГруппировка.ИспользуетсяАвтовыбор КАК ИспользуетсяАвтовыбор,
		|	втГруппировка.ХарактеристикиИспользуются КАК ХарактеристикиИспользуются,
		|	втГруппировка.ТипНоменклатуры КАК ТипНоменклатуры,
		|	втГруппировка.НаправлениеВыпуска КАК НаправлениеВыпуска,
		|	втГруппировка.ЦеховаяКладовая КАК ЦеховаяКладовая,
		|	втГруппировка.ТипСтатьи КАК ТипСтатьи,
		|	втГруппировка.АналитикаРасходовОбязательна КАК АналитикаРасходовОбязательна,
		|	втГруппировка.АналитикаРасходовЗаказРеализация КАК АналитикаРасходовЗаказРеализация,
		|	втГруппировка.НазначениеРедактирование КАК НазначениеРедактирование,
		|	втГруппировка.ДоляСтоимостиОбязательна КАК ДоляСтоимостиОбязательна,
		|	втГруппировка.ЕстьОшибкиЗаполненияДолиСтоимости КАК ЕстьОшибкиЗаполненияДолиСтоимости,
		|	втГруппировка.АналитикаАктивовПассивовНеИспользуется КАК АналитикаАктивовПассивовНеИспользуется,
		|	втГруппировка.НужноОкруглять КАК НужноОкруглять,
		|	втГруппировка.ДоступноОформлениеОтчетаДавальцу КАК ДоступноОформлениеОтчетаДавальцу,
		|	СУММА(втГруппировка.КоличествоУпаковок) КАК КоличествоУпаковок,
		|	СУММА(втГруппировка.Количество) КАК Количество,
		|	МАКСИМУМ(втГруппировка.Цена) КАК Цена,
		|	СУММА(втГруппировка.Сумма) КАК Сумма,
		|	СУММА(втГруппировка.ДоляСтоимостиПроцент) КАК ДоляСтоимостиПроцент
		|ИЗ
		...
		|	втГруппировка.Упаковка,
		|	втГруппировка.Получатель,
		|	втГруппировка.Назначение,
		|	втГруппировка.ДатаПроизводства,
		|	втГруппировка.СписатьНаРасходы,
		|	втГруппировка.СтатьяРасходов,
		|	втГруппировка.АналитикаРасходов,
		|	втГруппировка.АналитикаАктивовПассивов,
		|	втГруппировка.АналитикаУчетаНоменклатуры,
		|	втГруппировка.ОформитьОтчетДавальцу,
		|	втГруппировка.ОбработатьПоСпецификации,
		|	втГруппировка.Спецификация,
		|	втГруппировка.ЭтапПотребитель,
		|	втГруппировка.Период,
		|	втГруппировка.Подразделение,
		|	втГруппировка.НастройкаСчетовУчета,
		|	втГруппировка.ИспользуетсяАвтовыбор,
		|	втГруппировка.ХарактеристикиИспользуются,
		|	втГруппировка.ТипНоменклатуры,
		|	втГруппировка.НаправлениеВыпуска,
		|	втГруппировка.ЦеховаяКладовая,
		|	втГруппировка.ТипСтатьи,
		|	втГруппировка.АналитикаРасходовОбязательна,
		|	втГруппировка.АналитикаРасходовЗаказРеализация,
		|	втГруппировка.НазначениеРедактирование,
		|	втГруппировка.ДоляСтоимостиОбязательна,
		|	втГруппировка.ЕстьОшибкиЗаполненияДолиСтоимости,
		|	втГруппировка.АналитикаАктивовПассивовНеИспользуется,
		|	втГруппировка.НужноОкруглять,
		|	втГруппировка.ДоступноОформлениеОтчетаДавальцу";
	ТЗкЗагрузке = Объект.ВыходныеИзделия.Выгрузить();
	Запрос.УстановитьПараметр("ТЧГруппировка", ТЗкЗагрузке);
	РезультатЗапроса = Запрос.Выполнить();
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрокаТЧ, ТЗкЗагрузке[0]);
		ЗаполнитьЗначенияСвойств(НоваяСтрокаТЧ, ВыборкаДетальныеЗаписи);
		
		НоваяСтрокаТЧ.Отменено = Ложь;
		НоваяСтрокаТЧ.Произведено = Ложь;
	КонецЕсли;
КонецПроцедуры //-- ШмелевР 04.03.2026 ПРЗ.3
