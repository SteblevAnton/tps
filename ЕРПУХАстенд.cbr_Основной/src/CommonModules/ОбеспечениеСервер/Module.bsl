...
&После("УстановитьПараметрыЗапросаРасчетаСтатистики")
Процедура cbr_УстановитьПараметрыЗапросаРасчетаСтатистики(Запрос, ПериодКлассификации)
	//++ ШмелевР 27.01.2026 ЗКП.2
	Если Запрос.Параметры.Свойство("МестоЗапуска") 
		И Запрос.Параметры.МестоЗапуска = "ОбеспечениеПотребностейНоваяСхема" Тогда
		
		НачалоПотребления = Запрос.Параметры.НачалоИсторииПотребления;
		ОкончаниеПотребления = Запрос.Параметры.ОкончаниеИсторииПотребления;
		
		Периодичность = Перечисления.Периодичность.День;
		КоличествоПериодовКлассификации = (ОкончаниеПотребления - НачалоПотребления + 1) / 86400;
		
		РасчетныйПериод = ОбщегоНазначенияУТКлиентСервер.РасширенныйПериод(ТекущаяДатаСеанса(), Периодичность);
		
		Запрос.УстановитьПараметр("ПериодКлассификацииДатаНачала",    НачалоПотребления);
		Запрос.УстановитьПараметр("ПериодКлассификацииДатаОкончания", ОкончаниеПотребления);
