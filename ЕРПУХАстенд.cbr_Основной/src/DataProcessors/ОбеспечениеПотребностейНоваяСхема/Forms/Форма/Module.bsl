...
Процедура РассчитатьСтрокуТаблицыЗапасы(ИдентификаторСтроки, Отбор)
	...
	Если Таблица.Количество() > 0 Тогда
		...
		Если СтрокаЗапасы.МетодОбеспечения = МетодПоддержанияПоСтатистике Тогда
			...
			РезультатРасчетаКЗаказу = СтрокаЗапасы.СреднедневноеПотребление *
                                 СтрокаЗапасы.СрокПоставки *
                                 Товар.cbr_ПроцентКЗакупке / 100;
        	РезультатРасчетаКЗаказу = ЦенообразованиеКлиентСервер.ОкруглитьЦену(РезультатРасчетаКЗаказу, 1, 
        						  Перечисления.ВариантыОкругления.ВсегдаВПользуПредприятия);
        						  
			СтрокаЗапасы.КЗаказу = РезультатРасчетаКЗаказу + ВосполнениеСтраховогоЗапаса;		
		КонецЕсли;
		//-- ШмелевР 16.02.2026 ЗКП.1.2		
	КонецЕсли;
...
Функция ДалееКШагуЗапасы(Результат)
	...
	Для Каждого Товар Из Таблица Цикл
		...
		// ЗКП.КЗаказу
		Если СтрокаЗапасы.МетодОбеспечения = МетодПоддержанияПоСтатистике Тогда      	
        	РезультатКВосполнению = СтрокаЗапасы.СреднедневноеПотребление *
                                 СтрокаЗапасы.СрокПоставки *
                                 Товар.cbr_ПроцентКЗакупке / 100; 
			РезультатКВосполнению = ЦенообразованиеКлиентСервер.ОкруглитьЦену(РезультатКВосполнению, 1,Перечисления.ВариантыОкругления.ВсегдаВПользуПредприятия);     						  
			СтрокаЗапасы.КЗаказу = РезультатКВосполнению + Товар.СтраховойЗапас;
