...
Процедура cbr_ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если Статус = Перечисления.СтатусыЗаказовКлиентов.КОбеспечению И ЗначениеЗаполнено(Соглашение) Тогда
			...
			ТЗСостояниеОбъекта = Запрос.Выполнить().Выгрузить();
			
			Если ТЗСостояниеОбъекта.Количество() > 0 Тогда
				Для Каждого Стр Из ТЗСостояниеОбъекта Цикл
					Если Не Стр.СостояниеОбъекта = Перечисления.СостоянияСогласования.Утверждена Тогда
