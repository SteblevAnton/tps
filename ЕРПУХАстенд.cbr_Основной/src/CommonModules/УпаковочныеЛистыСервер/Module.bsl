...
Функция cbr_СинхронизироватьУпаковочныеЛистыСРасходнымОрдером(ОрдерСсылка, ИспользоватьАдресноеХранение) Экспорт
	...
	КонтрагентДляЛиста = Неопределено;
	
	Если ОрдерСсылка.ТоварыПоРаспоряжениям.Количество() Тогда
		Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Контрагент", ОрдерСсылка.ТоварыПоРаспоряжениям[0].Распоряжение.Метаданные()) Тогда 
