...
&НаКлиенте
Функция cbr_ДополнительныеСведенияЗаполненыНеправильно()
	...
	Если ДействиеПоРезультатамНайдено <> 0 Тогда
		...
		#Вставка
		//++AlopinAS ПРД1
		Если Клиент и НЕ ЗначениеЗаполнено(ЭтотОбъект.cbr_ОтрасльКонтрагента) Тогда
		ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо заполнить отрасль контрагента'"),ИспользуютсяТолькоПартнеры, "cbr_ОтрасльКонтрагента");	
		КонецЕсли;
		//--AlopinAS ПРД1
		#КонецВставки
...
Процедура cbr_ЗаписатьПартнераИСвязанныеДанные(СписокСозданных)
	...
	Если ДействиеПоРезультатамНайдено <> 0 Тогда
		...
		ОбъектПартнер.cbr_ОтрасльКонтрагента = ЭтотОбъект.cbr_ОтрасльКонтрагента;
...
//++AlopinAS ПРД.1
&НаКлиенте
Процедура cbr_ОтрасльКонтрагентаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЭтотОбъект.Клиент и Не ЭтотОбъект.Поставщик Тогда
	...
	ИначеЕсли ЭтотОбъект.Поставщик и Не ЭтотОбъект.Клиент Тогда	
		Элемент = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		Элемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Поставщик");
		Элемент.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		Элемент.Использование = Истина;
		Элемент.ПравоеЗначение = Истина;
	ИначеЕсли Не ЭтотОбъект.Клиент и Не ЭтотОбъект.Поставщик Тогда
		Элемент = ГруппаОтбора.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		Элемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Клиент");
		Элемент.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		Элемент.Использование = Истина;
		...
		Элемент.Использование = Истина;
		Элемент.ПравоеЗначение = Ложь;
	ИначеЕсли ЭтотОбъект.Поставщик и ЭтотОбъект.Клиент Тогда 
...
Процедура cbr_ОбработкаВыбораОтраслиКонтрагента(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	ЭтотОбъект.cbr_ОтрасльКонтрагента = РезультатЗакрытия;
