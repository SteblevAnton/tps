...
Асинх Процедура П3_КоличествоУпаковокФактПриИзмененииПосле(Элемент)
	...
	Для Каждого ТекущиеДанные Из Объект[ИмяТЧ] Цикл
		УправлениеПроизводствомКлиент.ВыходныеИзделияЭтапаПолучательПриИзменении(ТекущиеДанные, ИмяТЧ, ЭтаФорма, Объект, КэшированныеЗначения);
	КонецЦикла;
...
Процедура П3_НачатьЭтапПосле(Команда) Экспорт
	...
	Если НачатоВыполнениеЭтапа И ЗначениеЗаполнено(Объект.ФактическоеНачалоЭтапа) Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("П3_НачатьЭтапПослеЗавершение", ЭтаФорма), "Будут перезаполнены плановое время начала и завершения. Продолжить?", РежимДиалогаВопрос.ДаНет);
...
Процедура ПодготовитьФорму()
	...
	Команда = ФормыУХ.СоздатьКоманду(ЭтаФорма, ИмяКомандыОтменитьПересчет, "", ИмяКомандыОтменитьПересчет);
	...
	Если ИтогОбеспечения > 0 Тогда
		...
		ФормыУХ.СоздатьДобавитьРеквизитФормы(ЭтаФорма, 
		...
		ЭтаФорма.РеквизитИтогПоОбеспечению = ИтогОбеспечения;
...
Процедура ЗаполнитьСерииВОбеспечении()
	...
	ЕстьРеквизитРежимОтображенияСерийПоГодности = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтаФорма, "лимсРежимОтображенияСерийПоГодности");
	Если ЕстьРеквизитРежимОтображенияСерийПоГодности = Истина
		И ЭтаФорма.лимсРежимОтображенияСерийПоГодности = "ГодныеУсловноГодные" Тогда
	...
	Для Индекс = 1 По ВсегоСтрок Цикл
		...
		Если ЕстьНоваяСтрока И Не ЗначениеЗаполнено(СтрокаОбеспечения.Серия) Тогда
			...
			СлужебныеПараметры = ЭтаФорма.ДоступныеОстаткиПараметрыВстраивания.СлужебныеПараметры;
...
Процедура П3_СтатусПриИзменении(Элемент)
	...
	Если Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЭтаповПроизводства2_2.Завершен") Тогда
		...
		Если СуществующаяЗамывка.ЭтоПолуфабрикат Тогда		
			ПоказатьВопрос(Новый ОписаниеОповещения("П3_СтатусПриИзмененииЗавершение", ЭтаФорма), 
					   	   ТекстВопроса, 
...
Процедура П3_КоличествоУпаковокФактПриИзменении(Элемент)
	...
	Если Объект.КоличествоУпаковокФакт >= Объект.КоличествоУпаковокПлан И Объект.КоличествоУпаковокОтменено = 0 Тогда
		...
		Если СуществующаяЗамывка.ЭтоПолуфабрикат Тогда
			ПоказатьВопрос(Новый ОписаниеОповещения("П3_КоличествоУпаковокФактПриИзмененииЗавершение", ЭтаФорма), 
				   	   	   ТекстВопроса, 
				   	   	   РежимДиалогаВопрос.ДаНет);
		Иначе
			КоличествоУпаковокФактПриИзмененииНаСервере();
