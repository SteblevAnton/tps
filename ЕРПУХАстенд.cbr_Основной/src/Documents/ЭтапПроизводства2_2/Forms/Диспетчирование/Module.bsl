...
Функция cbr_ТекстЗапросаДинамическогоСпискаДиспетчированиеЭтапов()
	...
	|ВЫБРАТЬ
	|МАКСИМУМ(ДоступностьВидовРабочихЦентров.НомерСтроки) КАК НомерСтроки,
	|ДоступностьВидовРабочихЦентров.Регистратор КАК Регистратор
	|ПОМЕСТИТЬ ВТ_РабочиеЦентрыПоследняяСтрока
	|ИЗ
	|РегистрНакопления.ДоступностьВидовРабочихЦентров КАК ДоступностьВидовРабочихЦентров
	|ГДЕ
	|ТИПЗНАЧЕНИЯ(ДоступностьВидовРабочихЦентров.Регистратор) = ТИП(Документ.ЭтапПроизводства2_2)
	|
	|СГРУППИРОВАТЬ ПО
	|ДоступностьВидовРабочихЦентров.Регистратор
	|;
	|
	|ВЫБРАТЬ
	|ДоступностьВидовРабочихЦентров.ВидРабочегоЦентра КАК РабочийЦентр,
	|ДоступностьВидовРабочихЦентров.Регистратор КАК Регистратор,
	|ДоступностьВидовРабочихЦентров.НомерСтроки КАК НомерСтроки
	|ПОМЕСТИТЬ ВТ_РабочиеЦентры
	|ИЗ
	|ВТ_РабочиеЦентрыПоследняяСтрока КАК ВТ_РабочиеЦентрыПоследняяСтрока
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ДоступностьВидовРабочихЦентров КАК ДоступностьВидовРабочихЦентров
	|ПО (ВТ_РабочиеЦентрыПоследняяСтрока.Регистратор = ДоступностьВидовРабочихЦентров.Регистратор)
	|И (ДоступностьВидовРабочихЦентров.НомерСтроки = ВТ_РабочиеЦентрыПоследняяСтрока.НомерСтроки)
	|;
	|
	//-- cbr-larkovgeorg 09.02.26 ПРЗ.6
	|ВЫБРАТЬ
	...
	// КОНЕЦ Османов Н.М. 26.03.2025
	//++ cbr-larkovgeorg 09.02.26 ПРЗ.6
	|ВТ_РабочиеЦентры.РабочийЦентр КАК РабочийЦентр,
	//-- cbr-larkovgeorg 09.02.26 ПРЗ.6 
	#КонецВставки
	...
	#Вставка
	//++ cbr-larkovgeorg 09.02.26 ПРЗ.6
	| ЛЕВОЕ СОЕДИНЕНИЕ ВТ_РабочиеЦентры КАК ВТ_РабочиеЦентры
	| ПО (Этапы.Ссылка = ВТ_РабочиеЦентры.Регистратор)
	//-- cbr-larkovgeorg 09.02.26 ПРЗ.6
	#КонецВставки
