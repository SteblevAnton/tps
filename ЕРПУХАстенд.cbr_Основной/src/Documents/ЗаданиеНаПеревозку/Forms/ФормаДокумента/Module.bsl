...
Процедура cbr_МаршрутОбъемПриИзменении(Элемент)
	...
	Объект.cbr_КоличествоПаллет = cbr_ПолучитьИтоговоеКоличество();
	//--cbr-larkovgeorg 26.02.2026 СКЛ.12
КонецПроцедуры
...
&После("ПослеПодбораРаспоряженийСервер")
&НаСервере
Процедура cbr_ПослеПодбораРаспоряженийСервер(СтруктураВозврата)
	//++cbr-larkovgeorg 26.02.2026 СКЛ.12
	Объект.cbr_КоличествоПаллет = cbr_ПолучитьИтоговоеКоличество();
	//--cbr-larkovgeorg 26.02.2026 СКЛ.12
КонецПроцедуры

&После("МаршрутПередУдалениемНаСервере")
&НаСервере
Процедура cbr_МаршрутПередУдалениемНаСервере(Знач ВыделенныеСтроки)
	//++cbr-larkovgeorg 26.02.2026 СКЛ.12
	Объект.cbr_КоличествоПаллет = cbr_ПолучитьИтоговоеКоличество();
	//--cbr-larkovgeorg 26.02.2026 СКЛ.12
КонецПроцедуры
...
&НаСервере
Функция cbr_ПолучитьИтоговоеКоличество()
	//++cbr-larkovgeorg 26.02.2026 СКЛ.12
	Возврат Документы.ЗаданиеНаПеревозку.cbr_ИтоговоеКоличествоПаллетов(Объект.Маршрут);
	//--cbr-larkovgeorg 26.02.2026 СКЛ.12	
КонецФункции
#КонецОбласти