...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	//-- Тусеев Н.Ю. 27.01.2026 ПРД7 
		...
		"Объект.Товары.cbr_СтрокаИзСоглашенияСКлиентом",
	...
	ИначеЕсли ИмяФормы = "Документ.ЗаказНаПеремещение.Форма.ФормаДокумента" Тогда
		...
		//++АшаеваАН 13.02.26 СКЛ.3
		НовыйЭлемент = ФормыУХ.СоздатьПолеФормы(Форма.Элементы, "cbr_ЗаказПоставщику",, "Объект.Товары.cbr_ЗаказПоставщику",
		...
		ДобавляемыеРеквизиты = Новый Массив;
		ФормыУХ.СоздатьДобавитьРеквизитФормы(Форма, ДобавляемыеРеквизиты, "cbr_СкладРаспределения", Новый ОписаниеТипов("Булево"));
		Форма.ИзменитьРеквизиты(ДобавляемыеРеквизиты);
		
		Если ЗначениеЗаполнено(Форма.Объект.СкладОтправитель) Тогда
			Форма.cbr_СкладРаспределения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Форма.Объект.СкладОтправитель, "cbr_СкладРаспределения");
		КонецЕсли;
		...
		"ТоварыВариантОбеспечения", "ТолькоПросмотр", Истина);
		//--АшаеваАН 13.02.26 СКЛ.3
	...
	ИначеЕсли ИмяФормы = "Документ.УпаковочныйЛист.Форма.ФормаДокумента" Тогда
		...
		ФормыУХ.СоздатьПолеФормы(Форма.Элементы, "cbr_Паллет", "Паллет",
			"Объект.Товары.cbr_Паллет", ВидПоляФормы.ПолеВвода, Форма.Элементы.Товары);
		...
		ФормыУХ.СоздатьПолеФормы(Форма.Элементы, "cbr_Контрагент", "Контрагент",
			"Объект.cbr_Контрагент", ВидПоляФормы.ПолеВвода, Форма.Элементы.ШапкаНиз);
		...
		Если НайденныеСтроки.Количество() <> 0 Тогда
			...
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "ТоварыРаспаковать", "Доступность", Ложь);
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "ТоварыУпаковать", "Доступность", Ложь);
			
		Иначе 
			
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "ТоварыРаспаковать", "Доступность", Истина);
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Форма.Элементы, "ТоварыУпаковать", "Доступность", Истина);
