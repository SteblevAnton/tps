...
Процедура cbr_ПриОткрытииПосле(Отказ)
	//++ Мохов ПРЗ.18
	//Если -1, значит что таймер активен
...
&НаСервере
Процедура cbr_ПередЗаписьюНаСервереПеред(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	//++ Мохов ПРЗ.18 12.03.2026
	Если Объект.Статус = Перечисления.СтатусыЭтаповПроизводства2_2.cbr_НаКонтроле 
	// И Объект.cbr_ДатаДокументКотнроля 
	Тогда
		Объект.cbr_ВремяКонтроля = -1;	
	КонецЕсли;	
	//-- Мохов ПРЗ.18 12.03.2026
КонецПроцедуры
