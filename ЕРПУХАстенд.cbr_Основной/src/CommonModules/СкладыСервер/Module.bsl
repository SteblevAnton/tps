...
Процедура cbr_ПодобратьЯчейкиДляОтбора(МенеджерВременныхТаблиц, Склад, Помещение, СтруктураТаблиц, ОтборТары)
	...
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ИспользоватьПравилаОтбора,");
			...
			Пока ВыборкаПоХарактеристикам.Следующий() Цикл
				...
				Пока ВыборкаПоНазначению.Следующий() Цикл
					...
					Пока ВыборкаПоСериям.Следующий() Цикл
						...
						ОстаткиСборкиНеСухих = 0;

						ИспользоватьПравилаОтбора = Ложь;
						КоличествоКОтбору = 0;
						КоличествоОстаток = 0;
						ЧислительУпаковки = 0;

						Если ВыборкаПоЯчейкам.Следующий() Тогда
							ИспользоватьПравилаОтбора = ВыборкаПоЯчейкам.ИспользоватьПравилаОтбора;
							КоличествоКОтбору = ВыборкаПоЯчейкам.Количество;
							КоличествоОстаток = КоличествоКОтбору;
						КонецЕсли;
						...
						Пока ВыборкаПоЯчейкам.Следующий() Цикл
							...
							Иначе
								СвободныйОстаток = ВыборкаПоЯчейкам.СвободныйОстаток;
							КонецЕсли;
														
							Если ВыборкаПоЯчейкам.СборкаСухих Тогда
								ОстаткиСборкаСухих = ОстаткиСборкаСухих + СвободныйОстаток;
							Иначе
								ОстаткиСборкиНеСухих = ОстаткиСборкиНеСухих + СвободныйОстаток;
							КонецЕсли;
						...
						Если ИспользоватьПравилаОтбора Тогда

							Если ЧислительУпаковки <> 0 Тогда
								КОбеспечению = КоличествоКОтбору / ЧислительУпаковки;
								КОбеспечениюЦелое = Цел(КОбеспечению);
							...
							ИначеЕсли КОбеспечению > 0 И КОбеспечениюОстаток > 0 И ОстаткиСборкаСухих >= КОбеспечениюОстаток Тогда
								...
								КоличествоКОтборуСухих = КОбеспечениюОстаток;
								КоличествоКОтборуНеСухих = КОбеспечениюЦелое;
						...
						Если ИспользоватьПравилаОтбора Тогда
							...
							Иначе
								...
								КоличествоОстаток = КоличествоОстаток - КоличествоКОтборуСухих - КоличествоКОтборуНеСухих;
								КоличествоОстаток = ?(КоличествоОстаток < 0, 0, КоличествоОстаток);
								
							КонецЕсли;

						КонецЕсли;
						
						Если Не ИспользоватьПравилаОтбора Тогда
							КоличествоОстаток = 0;
						КонецЕсли;
						//-- СкребковЮМ 16.02.26 СКЛ.13
                        #КонецВставки
						...
						Пока ВыборкаПоЯчейкам.Следующий() Цикл
							...
							Если Не Перемещение Тогда
								...
								Если ЗначениеЗаполнено(ВыборкаПоЯчейкам.Упаковка)
									И КоличествоКОтбору < ВыборкаПоЯчейкам.КоличествоВУпаковке
									И КоличествоОстаток < ВыборкаПоЯчейкам.КоличествоВУпаковке Тогда
									Продолжить;
								КонецЕсли;
							...
							Если ЗначениеЗаполнено(ВыборкаПоЯчейкам.Ячейка)
								...
								Если КоличествоОстаток > 0 Тогда
									...
									Если ЗначениеЗаполнено(ВыборкаПоЯчейкам.Упаковка) Тогда
										КоличествоИзЯчейки = Мин(Цел(КоличествоОстаток / ВыборкаПоЯчейкам.КоличествоВУпаковке), СтрокаРасхода.КоличествоУпаковок);
									Иначе
										КоличествоИзЯчейки = Мин(КоличествоОстаток, СтрокаРасхода.КоличествоУпаковок);
									КонецЕсли;
									...
									Если КоличествоИзЯчейки > 0 Тогда
										...
										СтрокаРасхода.КоличествоУпаковок = СтрокаРасхода.КоличествоУпаковок - КоличествоИзЯчейки;
										КоличествоОстаток = КоличествоОстаток - КоличествоИзЯчейки * ВыборкаПоЯчейкам.КоличествоВУпаковке;
							...
							#Вставка
							//++ СкребковЮМ 16.02.26 СКЛ.13							
							Если ИспользоватьПравилаОтбора И СборкаСухих = Неопределено Тогда

								Если КоличествоКОтборуСухих + КоличествоКОтборуНеСухих = 0 И КоличествоОстаток = 0 Тогда
									Прервать;
								КонецЕсли;
							...
							Иначе
								...
								Если ИспользоватьПравилаОтбора Тогда
									...
									Если КоличествоКОтбору = 0 И КоличествоОстаток = 0 Тогда
										Прервать;
									КонецЕсли;
						...
						Если Перемещение И КоличествоКОтборуСухих > 0 Тогда
							...
							Если ТаблицаПеремещенияНеСухие.Количество() > 0 И ТаблицаПеремещенияСухие.Количество() > 0 Тогда

								Для Каждого СтрокаПеремещениеНеСухие Из ТаблицаПеремещенияНеСухие Цикл
									
									Если КоличествоКПереносуСухих = 0 Тогда
										Прервать;
									КонецЕсли;
									...
									Если ЗначениеЗаполнено(СтрокаПеремещениеНеСухие.Упаковка) Тогда
										КоличествоКПеремещению = Мин(Окр(КоличествоКПереносуСухих / СтрокаПеремещениеНеСухие.КоличествоВУпаковке + 0.5, 0, 0),
									...
									ТаблицаПеремещенияСухие[0].Серия = СтрокаПеремещениеНеСухие. Серия;
						...
						Если ИспользоватьПравилаОтбора И СборкаСухих = Неопределено Тогда
							...
							Если КоличествоКОтборуСухих + КоличествоКОтборуНеСухих <> 0 ИЛИ КоличествоОстаток <> 0 Тогда

								СтрокаОшибки = СтруктураТаблиц.ТаблицаОшибок.Добавить();
								ЗаполнитьЗначенияСвойств(СтрокаОшибки, ВыборкаПоНазначению);
								СтрокаОшибки.Количество = КоличествоКОтборуСухих + КоличествоКОтборуНеСухих + КоличествоОстаток;
						...
						Иначе
							...
							Если КоличествоКОтбору <> 0 ИЛИ КоличествоОстаток <> 0 Тогда

								СтрокаОшибки = СтруктураТаблиц.ТаблицаОшибок.Добавить();
								ЗаполнитьЗначенияСвойств(СтрокаОшибки, ВыборкаПоНазначению);
								СтрокаОшибки.Количество = КоличествоКОтбору + КоличествоОстаток;
