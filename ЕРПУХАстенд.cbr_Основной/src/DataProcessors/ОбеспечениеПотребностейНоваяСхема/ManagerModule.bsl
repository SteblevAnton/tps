...
Процедура ТаблицаСпособовОбеспечения(Параметры, АдресРезультата) Экспорт
	...
	СхемаКомпоновкиДанных = Обработки.ОбеспечениеПотребностейНоваяСхема.ПолучитьМакет("МакетКомпоновкиДляСерверныхОтборов");
	...
	Запрос.Текст = Обработки.ОбеспечениеПотребностейНоваяСхема.СформироватьТекстЗапросаАктуализацииСпособовОбеспечения();
	...
	Запрос.Текст = Обработки.ОбеспечениеПотребностейНоваяСхема.СформироватьТекстЗапросаРеквизитовСпособаОбеспечения(Неопределено);
...
Функция УровеньЗапасаИКартинкаСменитьМетод(Товар) Экспорт
	...
	ТочкаЗаказаКартинка   = Товар.Остаток + Товар.ЗаказыКпоступлению - Товар.МинимальныйЗапас < 0;
...
Функция КоличествоКЗаказуДляПоддержанияЗапаса(Товар) Экспорт
	...
	// НАЧАЛО Османов Н.М. 28.03.2025 
	ОбщийОстатокПоПозиции = Товар.Остаток + Товар.ЗаказыКпоступлению;

	Если ОбщийОстатокПоПозиции >= Товар.МинимальныйЗапас Тогда
		КЗаказу = 0;		
	Иначе
		КЗаказу = Товар.МаксимальныйЗапас - ОбщийОстатокПоПозиции; 
	КонецЕсли;
	// КОНЕЦ Османов Н.М. 28.03.2025 
	
	ДоТочкиЗаказа = Товар.Остаток - Товар.МинимальныйЗапас;
	// НАЧАЛО Османов Н.М. 28.03.2025 
	//// если Товар.Остаток + Товар.ЗаказыКпоступлению > Товар.МинимальныйЗапас, то ничего не заказываем
	//// если Товар.Остаток + Товар.ЗаказыКпоступлению < Товар.МинимальныйЗапас, то дозаказываем до максимального
	//
...
Функция СформироватьТекстЗапросаРеквизитовСпособаОбеспечения(ИмяВременнойТаблицы = Неопределено, ИмяИсходнойТаблицы = Неопределено) Экспорт
	...
	|	ЕСТЬNULL(СпрСпособ.УдалитьДатаСледующейПоставки, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаСледующейПоставки,
	
	|	ВЫБОР
	|		КОГДА СпрСпособ.ФормироватьЗаказыПоРасписанию И СпрСпособ.ПерваяДатаВГрафикеПоставок > ДАТАВРЕМЯ(1, 1, 1) ТОГДА
	|			СпрСпособ.ПерваяДатаВГрафикеПоставок //есть актуальная дата поставки по графику
	...
	|	ВЫБОР
	|		КОГДА СпрСпособ.ФормироватьЗаказыПоРасписанию И СпрСпособ.УдалитьПлановаяДатаЗаказа = &НачалоПериода ТОГДА
	|			&НачалоПериода
	|		КОГДА &КалендарьПредприятия = ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
	|			И СпрСпособ.ФормироватьЗаказыПоРасписанию
	|			И СпрСпособ.ПерваяДатаВГрафикеПоставок > ДАТАВРЕМЯ(1, 1, 1) ТОГДА //календари не используются
	|			ДОБАВИТЬКДАТЕ(СпрСпособ.ПерваяДатаВГрафикеПоставок, ДЕНЬ, - СпрСпособ.СрокИсполненияЗаказа)
	|		ИНАЧЕ
	...
	|		КОГДА СпрСпособ.ФормироватьЗаказыПоРасписанию И СпрСпособ.УдалитьПлановаяДатаЗаказа = &НачалоПериода ТОГДА
	|			ИСТИНА
	|		КОГДА СпрСпособ.ФормироватьЗаказыПоРасписанию И СпрСпособ.ПерваяДатаВГрафикеПоставок > ДАТАВРЕМЯ(1, 1, 1)
	|			И &КалендарьПредприятия = ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
	|			И ВЫБОР КОГДА СпрСпособ.ФормироватьЗаказыПоРасписанию И СпрСпособ.ПерваяДатаВГрафикеПоставок > ДАТАВРЕМЯ(1, 1, 1) ТОГДА
	|						ДОБАВИТЬКДАТЕ(СпрСпособ.ПерваяДатаВГрафикеПоставок, ДЕНЬ, - СпрСпособ.СрокИсполненияЗаказа) <= &НачалоПериода
	|					ИНАЧЕ
	|						ЛОЖЬ
	|				КОНЕЦ
	|		ТОГДА
	...
	|			И СпрСпособ.ФормироватьЗаказыПоРасписанию
	|			И СпрСпособ.ПерваяДатаВГрафикеПоставок > ДАТАВРЕМЯ(1, 1, 1) ТОГДА //календари не используются
	|			РАЗНОСТЬДАТ(&НачалоПериода, СпрСпособ.ПерваяДатаВГрафикеПоставок, ДЕНЬ) - СпрСпособ.СрокИсполненияЗаказа
	|		ИНАЧЕ
	...
	|		КОГДА &КалендарьПредприятия <> ЗНАЧЕНИЕ(Справочник.Календари.ПустаяСсылка)
	|			И СпрСпособ.ФормироватьЗаказыПоРасписанию
	|			И СпрСпособ.ПерваяДатаВГрафикеПоставок > ДАТАВРЕМЯ(1, 1, 1)
	...
	|			И ПлановаяДатаПоставкиГрафик.ДатаГрафика = СпрСпособ.ПерваяДатаВГрафикеПоставок
	|			И ПлановаяДатаПоставкиГрафик.Год = ГОД(СпрСпособ.ПерваяДатаВГрафикеПоставок)
	...
	|			И СпрСпособ.ФормироватьЗаказыПоРасписанию
	|			И ПлановаяДатаЗаказаГрафик.Год = ГОД(СпрСпособ.ПерваяДатаВГрафикеПоставок)
	...
	|					ВЫБОР КОГДА СпрСпособ.ФормироватьЗаказыПоРасписанию И СпрСпособ.ПерваяДатаВГрафикеПоставок > ДАТАВРЕМЯ(1,1,1) ТОГДА
	|									ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(СпрСпособ.ПерваяДатаВГрафикеПоставок, ГОД), ДЕНЬ, -1)
	|								ИНАЧЕ
	|									NULL
	|							КОНЕЦ
	|			И ВсегоРабочихДнейВГодуПлановогоЗаказа.Год = ГОД(СпрСпособ.ПерваяДатаВГрафикеПоставок) - 1
	...
	|				- СпрСпособ.СрокИсполненияЗаказа
	|				+ ПлановаяДатаПоставкиГрафик.КоличествоДнейВГрафикеСНачалаГода
	|			И ДатаЗаказаМинусГод.Год = ГОД(СпрСпособ.ПерваяДатаВГрафикеПоставок) - 1
...
Функция СформироватьТекстЗапросаАктуализацииСпособовОбеспечения() Экспорт
	...
	|ГДЕ
	|	СпрСпособ.ФормироватьЗаказыПоРасписанию
	|	И СпрСпособ.УдалитьПлановаяДатаЗаказа < &НачалоПериода
	|	И (СпрСпособ.ПерваяДатаВГрафикеПоставок <> ДАТАВРЕМЯ(1, 1, 1)
	|		ИЛИ СпрСпособ.УдалитьДатаСледующейПоставки <> ДАТАВРЕМЯ(1, 1, 1))";
...
Процедура ЗаполнитьРеквизитыСпособаОбеспечения(ПараметрыЦиклаПоставки, НачалоПериода, АктуализироватьДаты) Экспорт
	...
	Если АктуализироватьДаты И ЗначениеЗаполнено(СпособОбеспечения) Тогда
		...
		|
		|/////////////////////////////////////////////////////////////////////
		|" + Обработки.ОбеспечениеПотребностейНоваяСхема.СформироватьТекстЗапросаАктуализацииСпособовОбеспечения();
...
Функция ПоляТаблицыЗаказы(Секции) Экспорт
	...
	Если СтрНайти(Секции, "РеквизитыОбеспечивающегоЗаказа") Тогда
		...
		Поля.Вставить("КЗаказу",             ОбщегоНазначенияУХ.ПолучитьОписаниеТиповЧисла(15, 3));
...
Процедура ТаблицаЗапасы(Параметры, АдресРезультата) Экспорт
	...
	Если Параметры.ТипОтбора = "ОтборПоЗначениям" Тогда
		...
		//	|	Способы.СпособОбеспечения.ФормироватьЗаказыПоРасписанию КАК ФормироватьПлановыеЗаказы,
	...
	ИначеЕсли  Параметры.ТипОтбора = "ОтборПоЗначениямСФормы" Тогда	//++ ШмелевР 16.02.2026 ЗКП.1.2
		ТаблицаСпособовОбеспечения = Новый ТаблицаЗначений;
		ТаблицаСпособовОбеспечения.Колонки.Добавить("СпособОбеспечения",  Новый ОписаниеТипов("СправочникСсылка.СпособыОбеспеченияПотребностей"));
		ТаблицаСпособовОбеспечения.Колонки.Добавить("ФормироватьПлановыеЗаказы", Новый ОписаниеТипов("Булево"));
		ТаблицаСпособовОбеспечения.Колонки.Добавить("ДатаПоставки", ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.Дата));
		
		НоваяСтрока = ТаблицаСпособовОбеспечения.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Параметры.Отбор);
		
		Параметры.Вставить("Отбор", ПолучитьИзВременногоХранилища(Параметры.КлючДанных));
		СхемаКомпоновкиДанных = Обработки.ОбеспечениеПотребностейНоваяСхема.ПолучитьМакет("МакетКомпоновкиДляСерверныхОтборов");
		АдресСхемыКомпоновкиДанных = ПоместитьВоВременноеХранилище(СхемаКомпоновкиДанных);
	...
	// НАЧАЛО Османов Н.М. 25.03.2025
	Запрос.Текст = ВременнаяТаблицаКВыполнению();
	Запрос.Выполнить();
	// КОНЕЦ Османов Н.М. 25.03.2025
	
	// НАЧАЛО Османов Н.М. 05.05.2025
	Запрос.Текст = ВременнаяТаблицаОстаткиПроизводства();
	Если Запрос.Параметры.Свойство("Склад") Тогда
		Запрос.УстановитьПараметр("СкладыПроизводства", Запрос.Параметры.Склад);	
	КонецЕсли;
	Запрос.Выполнить();
	// КОНЕЦ Османов Н.М. 05.05.2025
	
	// НАЧАЛО Османов Н.М. 06.05.2025 
	Запрос.Текст = ВременнаяТаблицаЗакупкиЗаПериод();
	Запрос.УстановитьПараметр("ХозяйственныеОперацииИсключения", РасчетСебестоимостиПрикладныеАлгоритмы.ХозяйственныеОперацииПеремещений());
	Запрос.УстановитьПараметр("НастройкаХозяйственнойОперации", ПредопределенноеЗначение("Справочник.НастройкиХозяйственныхОпераций.Закупки"));
	Запрос.Выполнить();
	// КОНЕЦ Османов Н.М. 06.05.2025
	
	// НАЧАЛО Османов Н.М. 13.05.2025 
	Запрос.Текст = ВременнаяТаблицаИсторияПотребленияПроизводстваЗаПериод();
	Запрос.Выполнить();
	// КОНЕЦ Османов Н.М. 13.05.2025
	
	//++ ШмелевР 27.01.2026 ЗКП.2
	Запрос.УстановитьПараметр("МестоЗапуска", "ОбеспечениеПотребностейНоваяСхема");
	//-- ШмелевР 27.01.2026 ЗКП.2
	...
	ТекстыЗапросов.Добавить(
		...
		|////////////////////////////////////////////////////////////////////////////////
		|");
	ТекстыЗапросов.Добавить(ВременнаяТаблицаДатПоставок());
	...
	ТекстыЗапросов.Добавить(ВременнаяТаблицаИнтерваловРаботыСкладов());
	...
	Текст =
		...
		|	КОНЕЦ КАК Остаток,
		|// НАЧАЛО Османов Н.М. 05.05.2025 
		|	ОстаткиПроизводства.Запас КАК ОстатокНаПроизводственномСкладе,
		|// КОНЕЦ Османов Н.М. 05.05.2025
		|// НАЧАЛО Османов Н.М. 06.05.2025 
		|	ВТ_ЗакупкиЗаПериод.ЗакупкиЗаПериод,
		|// КОНЕЦ Османов Н.М. 06.05.2025 
		|// НАЧАЛО Османов Н.М. 13.05.2025 
		|	ВТ_ИсторияПотребленияЗаПериод.КоличествоИсторияПроизводство,
		|// КОНЕЦ Османов Н.М. 13.05.2025
		|// НАЧАЛО Османов Н.М. 19.05.2025 
		|	Остатки.КОбеспечению КАК Обеспечение,
		|	ВЫБОР
		|		КОГДА ЗНАЧЕНИЕ(Перечисление.ОбеспечениеЗаказовПриПоддержанииЗапаса.ЗаСчетЗапасов) = ТоварныеОграничения.ОбеспечениеЗаказовПриПоддержанииЗапаса
		|			ТОГДА ЕСТЬNULL(Остатки.ВНаличии, 0)
		|		КОГДА ЦентрыРаспределительныхСетей.Номенклатура ЕСТЬ NULL
		...
		|				ИНАЧЕ 0
		|			КОНЕЦ
		|	КОНЕЦ - Остатки.КОбеспечению КАК ОстатокСОбеспечением,
		|// КОНЕЦ Османов Н.М. 19.05.2025 
		|	
		|	ВЫБОР
		|		КОГДА ЗНАЧЕНИЕ(Перечисление.ОбеспечениеЗаказовПриПоддержанииЗапаса.ЗаСчетЗапасов) = ТоварныеОграничения.ОбеспечениеЗаказовПриПоддержанииЗапаса
		|			ТОГДА 0
		...
		|	ЕСТЬNULL(Т.СпособОбеспечения.cbr_ПроцентКЗакупке,0)  КАК cbr_ПроцентКЗакупке,
		////-- ШмелевР 28.01.2026 ЗКП.2
		...
		|		// НАЧАЛО Османов Н.М. 26.02.2025
		|		 И Т.Склад          = Остатки.Склад
		|		// КОНЕЦ Османов Н.М. 26.02.2025
		|		 И Остатки.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|
		|		// НАЧАЛО Османов Н.М. 05.05.2025 
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ОстаткиПроизводства КАК ОстаткиПроизводства
		|		ПО Т.Номенклатура   = ОстаткиПроизводства.Номенклатура
		|		 И Т.Характеристика = ОстаткиПроизводства.Характеристика
		|		 И ОстаткиПроизводства.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
		|		// КОНЕЦ Османов Н.М. 05.05.2025 
		|
		|		// НАЧАЛО Османов Н.М. 06.05.2025 
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЗакупкиЗаПериод КАК ВТ_ЗакупкиЗаПериод
		|		ПО Т.Номенклатура   = ВТ_ЗакупкиЗаПериод.Номенклатура
		|		 И Т.Характеристика = ВТ_ЗакупкиЗаПериод.Характеристика
		|		// КОНЕЦ Османов Н.М. 06.05.2025 
		|
		|		// НАЧАЛО Османов Н.М. 13.05.2025 
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ИсторияПотребленияЗаПериод КАК ВТ_ИсторияПотребленияЗаПериод
		|		ПО Т.Номенклатура   = ВТ_ИсторияПотребленияЗаПериод.Номенклатура
		|		 И Т.Характеристика = ВТ_ИсторияПотребленияЗаПериод.Характеристика
		|		// КОНЕЦ Османов Н.М. 13.05.2025 
		|
...
Процедура ТаблицаПотребностейПоЗаказамИОстатков(Параметры, АдресРезультата) Экспорт
	...
	Если Параметры.ТипОтбора = "ОтборПоЗначениям" Тогда
		...
		ТекстыЗапросов.Добавить(
			...
			|ВЫБРАТЬ
			|	Способы.СпособОбеспечения                                           КАК СпособОбеспечения,
			|	ЕСТЬNULL(Способы.СпособОбеспечения.ФормироватьЗаказыПоРасписанию, ЛОЖЬ) КАК ФормироватьПлановыеЗаказы,
			|	NULL                                                                КАК ДатаПоставки
			|ПОМЕСТИТЬ ВтСпособыОбеспеченияПереопределенный
			|ИЗ
			|	ВтТовары КАК Способы
	...
	//++ШмелевР 11.02.2026 ЗКП.19
	ТекстыЗапросов[ТекстыЗапросов.Количество() - 1] = СтрЗаменить(ТекстыЗапросов[ТекстыЗапросов.Количество() - 1], 
		"Т.Номенклатура                                   КАК Номенклатура,",
		"Т.Номенклатура                                   КАК Номенклатура, 
		|	ЕСТЬNULL(ПоддерживаемостьОстатков.НеПоддерживаютсяОстатки, ЛОЖЬ) КАК cbr_ПоддерживаетсяОстаток,");
	...
	ТекстыЗапросов.Вставить(ТекстыЗапросов.Количество() - 1, ОбщегоНазначения.РазделительПакетаЗапросов());
	...
	Если Параметры.ТипОтбора <> "ОтборПоЗначениям" Тогда
		...
		ТекстыЗапросов.Добавить(ВременнаяТаблицаСвободныеОстатки(Истина, Истина));
		// КОНЕЦ Османов Н.М. 25.03.2025 
...
Процедура СформироватьДеревоЗаказов(Параметры, АдресРезультата) Экспорт
	...
	ИндексЗапасы = 0;
	Пока ИндексЗапасы < Параметры.Запасы.Количество() Цикл
		
		СтрокаЗапас = Параметры.Запасы[ИндексЗапасы];
...
Процедура УдалитьЗаказы(Параметры, АдресРезультата) Экспорт
	...
	Исключение
		...
		ИмяСобытияЖурнала = НСтр("ru = 'Формирование заказов: установка пометки удаления документов';
								|en = 'Order generation: set document deletion mark'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПолучитьЗапросСОтборамиКомпоновкиДанных(АдресСхемыКомпоновкиДанных, КомпоновщикНастроек, ИмяНабораДанных)
	...
	ОтборКомпоновки = НастройкиКомпоновки.Отбор;
	СписокСкладов = ЗначениеСпециальногоОтбора(ОтборКомпоновки, "СкладыОстатков");
	СписокСкладовОбеспечения = ЗначениеСпециальногоОтбора(ОтборКомпоновки, "СкладыОбеспечения");
	СписокСкладовПроизводства = ЗначениеСпециальногоОтбора(ОтборКомпоновки, "СкладыПроизводства");
	НачалоПериодаЗакупок = ЗначениеСпециальногоОтбора(ОтборКомпоновки, "НачалоПериодаЗакупок");
	НачалоПериодаЗакупок = ?(НачалоПериодаЗакупок = Неопределено, Дата(1, 1, 1), НачалоПериодаЗакупок.Дата);
	ОкончаниеПериодаЗакупок = ЗначениеСпециальногоОтбора(ОтборКомпоновки, "ОкончаниеПериодаЗакупок");
	ОкончаниеПериодаЗакупок = ?(ОкончаниеПериодаЗакупок = Неопределено, Дата(1, 1, 1), ОкончаниеПериодаЗакупок.Дата);
	// КОНЕЦ Османов Н.М. 26.02.2025
	// НАЧАЛО Османов Н.М. 13.05.2025 
	НачалоИсторииПотребления = ЗначениеСпециальногоОтбора(ОтборКомпоновки, "НачалоИсторииПотребления");
	НачалоИсторииПотребления = ?(НачалоИсторииПотребления = Неопределено, Дата(1, 1, 1), НачалоИсторииПотребления.Дата);
	ОкончаниеИсторииПотребления = ЗначениеСпециальногоОтбора(ОтборКомпоновки, "ОкончаниеИсторииПотребления");
	ОкончаниеИсторииПотребления = ?(ОкончаниеИсторииПотребления = Неопределено, Дата(1, 1, 1), ОкончаниеИсторииПотребления.Дата);
	// КОНЕЦ Османов Н.М. 13.05.2025 
	...
	// НАЧАЛО Османов Н.М. 26.02.2025 
	Запрос.УстановитьПараметр("СкладыОстатков", СписокСкладов);
	// КОНЕЦ Османов Н.М. 26.02.2025 
	// НАЧАЛО Османов Н.М. 18.04.2025 
	Запрос.УстановитьПараметр("СкладыОбеспечения", СписокСкладовОбеспечения);
	// КОНЕЦ Османов Н.М. 18.04.2025
	// НАЧАЛО Османов Н.М. 29.04.2025 
	Запрос.УстановитьПараметр("СкладыПроизводства", СписокСкладовПроизводства);
	// КОНЕЦ Османов Н.М. 29.04.2025
	// НАЧАЛО Османов Н.М. 06.05.2025 
	Запрос.УстановитьПараметр("НачалоПериодаЗакупок", НачалоПериодаЗакупок);
	Запрос.УстановитьПараметр("ОкончаниеПериодаЗакупок", ОкончаниеПериодаЗакупок);
	// КОНЕЦ Османов Н.М. 06.05.2025
	// НАЧАЛО Османов Н.М. 13.05.2025 
	Запрос.УстановитьПараметр("НачалоИсторииПотребления", НачалоИсторииПотребления);
	Запрос.УстановитьПараметр("ОкончаниеИсторииПотребления", ОкончаниеИсторииПотребления);
	// КОНЕЦ Османов Н.М. 13.05.2025 
...
//-- НЕ УТКА
	...
	Иначе
		...
		ТекстСпособыОбеспечения = ТекстСпособыОбеспечения + ВременнаяТаблицаДатПоставок();
...
Функция ВременнаяТаблицаГраницыОбеспечиваемогоПериода()
	...
	ТекстЗапроса = ТекстЗапроса + ТекстЗапросаДатГрафика();
...
Функция ВременнаяТаблицаСвободныеОстатки(ВТаблицеОтбораЕстьНазначения, ЗаВычетомКВыполнению = Ложь)
	...
	Текст =
		...
		|	Остатки.Характеристика КАК Характеристика,
		|	Остатки.Назначение КАК Назначение,
		|	СУММА(Остатки.Запас) КАК Запас,
		|	СУММА(Остатки.Свободно - ЕСТЬNULL(КОбеспечениюВсехСкладов.ОбеспеченоСоСклада, 0)) КАК Свободно,
		|	СУММА(Остатки.Резерв) КАК ВРезерве
		...
		|	Т.Характеристика                КАК Характеристика,
		|	Т.Назначение                    КАК Назначение,
		|	СУММА(КОбеспечению)				КАК КОбеспечению,
		...
		|	ОБЪЕДИНИТЬ ВСЕ
		|
		|// НЕ УДАЛЯТЬ ПОДСТАНОВКА ЗНАЧЕНИЙ
		|	
		|	ВЫБРАТЬ
		|		Т.Номенклатура                    КАК Номенклатура,
		|		Т.Характеристика                  КАК Характеристика,
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	Т.Номенклатура, Т.Характеристика, Т.Назначение, Т.Склад
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура, Характеристика, Назначение, Склад
		|;
	...
	// НАЧАЛО Османов Н.М. 25.03.2025 
	Если Не ЗаВычетомКВыполнению Тогда
		ТекстПодЗапроса = "
		|	ВЫБРАТЬ
		|		Т.Номенклатура,
		|		Т.Характеристика,
		...
		|		0
		|	ИЗ
		|		ВТ_ТаблицаКОбеспечению КАК Т
		|
		|	ОБЪЕДИНИТЬ ВСЕ
		|";
		Текст = СтрЗаменить(Текст, "// НЕ УДАЛЯТЬ ПОДСТАНОВКА ЗНАЧЕНИЙ", ТекстПодЗапроса);
	КонецЕсли;
	// КОНЕЦ Османов Н.М. 25.03.2025 
	Тексты.Добавить(Текст);
	Текст = СтрСоединить(Тексты, ОбщегоНазначения.РазделительПакетаЗапросов());
...
Процедура РассчитатьДатыПоставокДляПроизводства(Таблица)
	...
	ТаблицаДляРасчета.Колонки.Добавить("Индекс", ОбщегоНазначенияУХ.ПолучитьОписаниеТиповЧисла(15, 0));
	...
	Запрос.Текст =
		...
		|;
		|/////////////////////////////////////////////////
		|" + ТекстЗапросаДатГрафика(Неопределено, Ложь) + "
		|ВЫБРАТЬ
...
Функция ЗначениеСпециальногоОтбора(ОтборКомпоновки, НаименованиеОтбора)
	
	РезультатФункции = Неопределено;
	
	МассивЭлементов = ОбщегоНазначенияКлиентСервер.НайтиЭлементыИГруппыОтбора(ОтборКомпоновки, НаименованиеОтбора);
	Для Каждого ЭлементМассива Из МассивЭлементов Цикл
		Если ЭлементМассива.Использование Тогда
			РезультатФункции = ЭлементМассива.ПравоеЗначение;
		КонецЕсли;
...
// Османов Н.М. 25.03.2025
Функция ВременнаяТаблицаКВыполнению()
	...
	|ПОМЕСТИТЬ ЗапасыИПотребностиВрем
	|ИЗ
	|	РегистрНакопления.ЗапасыИПотребности.Остатки(
	|			,
	|			Склад ССЫЛКА Справочник.Склады
	|				И Склад В (&СкладыОбеспечения)) КАК ИнформацияОДоступности
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура,
	|	Характеристика,
	...
	|ПОМЕСТИТЬ ТоварыКОтгрузкеВрем
	|ИЗ
	|	РегистрНакопления.ТоварыКОтгрузке.Остатки(
	|			,
	|			 Склад В (&СкладыОбеспечения)) КАК ТоварыКОтгрузке
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|	0 КАК ВРезерве,
	|	0 КАК Доступно,
	|	ИнформацияОДоступности.Зарезервировано КАК КОбеспечению
	|ПОМЕСТИТЬ ТаблицаДанныхОтчетаВрем
	|ИЗ
	...
	|	И ИнформацияОДоступности.Склад В(&СкладыОбеспечения)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	0
	|ИЗ
	|	ЗапасыИПотребностиВрем КАК ИнформацияОДоступности
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ ТаблицаКРаспределениюВрем
	|ИЗ
	|	ТаблицаДанныхОтчетаВрем КАК Таблица
	|
	|СГРУППИРОВАТЬ ПО
	|	Таблица.Номенклатура,
	|	Таблица.Характеристика,
	...
	|	Набор.Назначение КАК Назначение,
	|	СУММА(Набор.КОбеспечению) КАК КОбеспечению
	|ПОМЕСТИТЬ ВТ_ТаблицаКОбеспечению
	|ИЗ
	|	(ВЫБРАТЬ
	|		Таблица.Номенклатура КАК Номенклатура,
	|		Таблица.Характеристика КАК Характеристика,
	...
	|			0
	|		ИЗ
	|			ТаблицаКРаспределениюВрем КАК Таблица) КАК Таблица
	|	
	|	СГРУППИРОВАТЬ ПО
	|		Таблица.Номенклатура,
	|		Таблица.Характеристика,
	...
	|	
	|	ИМЕЮЩИЕ
	|		СУММА(Таблица.КОбеспечению - Таблица.ВРезерве) <> 0
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		План.Склад,
	|		План.Назначение,
	|		План.Требуется - План.РаспределеноИзЗапасовОбособленно - План.РаспределеноИзПартийОбособленно
	|	ИЗ
	|		РегистрСведений.СтруктураЗаказа КАК План
	|	ГДЕ
	|		План.Склад ССЫЛКА Справочник.Склады
	|		И НЕ План.ВидСтроки В (ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.Продукция), ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.ПроизводствоПродукции))
	|		И (План.Требуется <> 0
	|				ИЛИ План.РаспределеноИзЗапасовОбособленно <> 0
	|				ИЛИ План.РаспределеноИзПартийОбособленно <> 0)
	|		И План.Склад В(&СкладыОбеспечения)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		План.РаспределеноИзЗапасовОбособленно + План.РаспределеноИзПартийОбособленно
	|	ИЗ
	|		РегистрСведений.СтруктураЗаказа КАК План
	|	ГДЕ
	|		План.Склад ССЫЛКА Справочник.Склады
	|		И НЕ План.ВидСтроки В (ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.Продукция), ЗНАЧЕНИЕ(Перечисление.ВидыСтрокСтруктурыЗаказаНаПроизводство.ПроизводствоПродукции))
	|		И (План.РаспределеноИзЗапасовОбособленно <> 0
	|				ИЛИ План.РаспределеноИзПартийОбособленно <> 0)
	|		И План.Склад В(&СкладыОбеспечения)) КАК Набор
	|
	|СГРУППИРОВАТЬ ПО
	|	Набор.Номенклатура,
	|	Набор.Характеристика,
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ЗапасыИПотребностиВрем
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ТоварыКОтгрузкеВрем
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ТаблицаДанныхОтчетаВрем
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ТаблицаКРаспределениюВрем";
	
	Возврат ТекстЗапроса;
	
КонецФункции

Функция ВременнаяТаблицаОстаткиПроизводства()
	...
	Текст =
		...
		|	Запасы.Назначение КАК Назначение,
		|	Запасы.ВНаличииОстаток КАК Запас
		|ПОМЕСТИТЬ ВТ_ОстаткиПроизводства
		|ИЗ
		|	РегистрНакопления.ЗапасыИПотребности.Остатки(,
		|		Склад ССЫЛКА Справочник.Склады
		|			И Склад В (&СкладыПроизводства)) КАК Запасы
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура, Характеристика, Склад, Назначение";
...
Функция ВременнаяТаблицаЗакупкиЗаПериод()
	...
	Текст =
		...
		|ПОМЕСТИТЬ ВТ_ЗакупкиЗаПериодВременно
		|ИЗ
		|	РегистрНакопления.СебестоимостьТоваров КАК СебестоимостьТоваров
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НастройкиХозяйственныхОпераций КАК НастройкиХозяйственныхОпераций
		|		ПО СебестоимостьТоваров.НастройкаХозяйственнойОперации = НастройкиХозяйственныхОпераций.Ссылка
		|ГДЕ
		|	СебестоимостьТоваров.Период МЕЖДУ &НачалоПериодаЗакупок И &ОкончаниеПериодаЗакупок
		|	И НЕ СебестоимостьТоваров.ХозяйственнаяОперация В (&ХозяйственныеОперацииИсключения)
		|	И СебестоимостьТоваров.НастройкаХозяйственнойОперации В ИЕРАРХИИ (&НастройкаХозяйственнойОперации)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ЗакупкиЗаПериодВременно.Номенклатура КАК Номенклатура,
		|	ВТ_ЗакупкиЗаПериодВременно.Характеристика КАК Характеристика,
		|	СУММА(ВТ_ЗакупкиЗаПериодВременно.КоличествоПриход - ВТ_ЗакупкиЗаПериодВременно.КоличествоРасход) КАК ЗакупкиЗаПериод
		|ПОМЕСТИТЬ ВТ_ЗакупкиЗаПериод
		|ИЗ
		|	ВТ_ЗакупкиЗаПериодВременно КАК ВТ_ЗакупкиЗаПериодВременно
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ_ЗакупкиЗаПериодВременно.Номенклатура,
		|	ВТ_ЗакупкиЗаПериодВременно.Характеристика";
	
	Возврат Текст;
	
КонецФункции

Функция ВременнаяТаблицаИсторияПотребленияПроизводстваЗаПериод()
	
	Текст =
		"ВЫБРАТЬ
		|	СебестоимостьТоваров.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура,
		|	СебестоимостьТоваров.АналитикаУчетаНоменклатуры.Характеристика КАК Характеристика,
		|	СУММА(СебестоимостьТоваров.Количество) КАК КоличествоИсторияПроизводство
		|ПОМЕСТИТЬ ВТ_ИсторияПотребленияЗаПериод
		|ИЗ
		|	РегистрНакопления.СебестоимостьТоваров КАК СебестоимостьТоваров
		|ГДЕ
		|	СебестоимостьТоваров.Период МЕЖДУ &НачалоИсторииПотребления И &ОкончаниеИсторииПотребления
		|	И СебестоимостьТоваров.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Потребление)
		|	И СебестоимостьТоваров.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаМатериаловВПроизводство)
		|СГРУППИРОВАТЬ ПО
...
Функция ВременнаяТаблицаДатПоставок()
	...
	ТекстЗапроса = ТекстЗапроса +
		...
		|
		|	ЕСТЬNULL(Т.Ссылка.ФормироватьЗаказыПоРасписанию, ЛОЖЬ) КАК ФормироватьПлановыеЗаказы,
		|	Т.Ссылка.УдалитьПлановаяДатаЗаказа                 КАК ПлановаяДатаЗаказа,
		|	Т.Ссылка.ПерваяДатаВГрафикеПоставок                КАК ПлановаяДатаПоставки,
		|	Т.Ссылка.УдалитьДатаСледующейПоставки              КАК ДатаСледующейПоставки,
...
Функция ТекстЗапросаДатГрафика(Разделы = Неопределено, ИскатьВперед = Истина)
