...
Процедура ОткрытьМетодОбеспечения(ИмяТаблицы)
	...
	Поставка = Элементы[ИмяТаблицы].ТекущиеДанные;
	КлючНоменклатуры = Новый Структура("Номенклатура, Характеристика, Склад, ЭтоРабота, НачалоИсторииПотребления, ОкончаниеИсторииПотребления");
	КлючНоменклатуры.ЭтоРабота = Ложь;
	ЗаполнитьЗначенияСвойств(КлючНоменклатуры, Поставка);

	РежимОткрытия = "Полный";

	ПараметрыФормы = Новый Структура("КлючПотребности, РассчитыватьСтатистику, РежимОткрытия, МестоЗапуска",
		КлючНоменклатуры, Истина, РежимОткрытия, "ОбеспечениеПотребностейНоваяСхема");
    //++ ШмелевР 16.02.2026 ЗКП.1.2 
