...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ИмяФормы = "Справочник.Партнеры.Форма.ПомощникНового" Тогда
		...
		ДобавляемыеРеквизиты = Новый Массив;
		ФормыУХ.СоздатьДобавитьРеквизитФормы(Форма, ДобавляемыеРеквизиты, "cbr_ОтрасльКонтрагента", Новый ОписаниеТипов("СправочникСсылка.cbr_ОтраслиКонтрагентов"));
		
		Форма.ИзменитьРеквизиты(ДобавляемыеРеквизиты);
		
		ФормыУХ.СоздатьПолеФормы(Форма.Элементы, "cbr_ОтрасльКонтрагента", "Отрасль контрагента", "cbr_ОтрасльКонтрагента", ВидПоляФормы.ПолеВвода,
			Форма.Элементы.ГруппаСегментПервичныйИнтересКомпания);
	...
	ИначеЕсли ИмяФормы = "Документ.ЗаданиеНаПеревозку.Форма.ФормаДокумента" Тогда
		...
		Элементы.ЗаданиеВыполняет.СписокВыбора.Добавить(Перечисления.ТипыИсполнителейЗаданийНаПеревозку["cbr_НеУчаствуемВДоставке"]);
		
		ЭлементКоличествоПаллет = cbr_РаботаСФормами.ДобавитьПолеФормы(Форма, "cbr_КоличествоПаллет", 
		"Объект.cbr_КоличествоПаллет",, ВидПоляФормы.ПолеВвода, Элементы.ГруппаОсновное, Элементы.ДополнительнаяИнформация);
	...
	ИначеЕсли ИмяФормы = "Справочник.Склады.Форма.ФормаЭлемента" Тогда
		...
		ВидПоляФормы.ПолеВвода, Элементы.ЗаданияНаПеревозкуВРаботеОтбор, Элементы.КоманднаяПанельЗаданияНаПеревозкуВРаботе,
		"ПриИзменении");
		...
		ФормыУХ.СоздатьПолеФормы(Форма.Элементы, "cbr_СборкаСухих", "Сборка сухих",
			"Объект.cbr_СборкаСухих", ВидПоляФормы.ПолеФлажка, Форма.Элементы.cbr_ГруппаСборка);
		//-- СкребковЮМ 16.02.26 СКЛ.13
...
Процедура ДополнитьУсловноеОформление(Форма)
    ...
    Если ИмяФормы = "Документ.ЗаказКлиента.Форма.ФормаДокумента" Тогда
        ...
        ОтборЭлемента                = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
        ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Товары.cbr_СтрокаИзСоглашенияСКлиентом");
        ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
