...
Процедура cbr_ПриСозданииНаСервереПеред(Отказ, СтандартнаяОбработка)
	...
	ПараметрЗаказКлиентаСсылка = Неопределено;
	Если Параметры.Свойство("Ссылка") Тогда
		ПараметрЗаказКлиентаСсылка = Параметры.Ссылка;
	КонецЕсли;
	
	Если ТипЗнч(ПараметрЗаказКлиентаСсылка) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		Список.ТекстЗапроса =
		"ВЫБРАТЬ
