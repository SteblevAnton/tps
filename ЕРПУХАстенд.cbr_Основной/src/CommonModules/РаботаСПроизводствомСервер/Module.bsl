...
Функция УстановленЗапретОткрытияСпецификации(РесурснаяСпецификация) Экспорт 
	...
	Если Не УправлениеДоступом.ЕстьРоль("cbr_ПросмотрФормыСпецификацииНеНаФасовку") Тогда
...
Функция ЭтоПолуфабрикат(ПартияПроизводства) Экспорт
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("ПартияПроизводства", ПартияПроизводства);
	Запрос.УстановитьПараметр("ИдентификаторДляФормул", "ЭтоПолуфабрикатВидНоменклатуры");
	
	Запрос.Текст = "ВЫБРАТЬ
	|	СправочникНоменклатура.ВидНоменклатуры КАК ВидНоменклатуры
	|ПОМЕСТИТЬ втВидНоменклатуры
	|ИЗ
	...
	|				И СправочникНоменклатура.Ссылка = ПартииПроизводства.ОсновноеИзделиеНоменклатура
	|				И СправочникНоменклатура.ВидНоменклатуры <> ЗНАЧЕНИЕ(Справочник.ВидыНоменклатуры.ПустаяСсылка))	
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДополнительныеСведения.Значение) = ТИП(БУЛЕВО)
	|			ТОГДА ВЫРАЗИТЬ(ДополнительныеСведения.Значение КАК БУЛЕВО)
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоПолуфабрикат
	|ИЗ
	|	втВидНоменклатуры КАК втВидНоменклатуры
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДополнительныеСведения КАК ДополнительныеСведения
	|		ПО втВидНоменклатуры.ВидНоменклатуры = ДополнительныеСведения.Объект
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		Возврат ВыборкаДетальныеЗаписи.ЭтоПолуфабрикат;	
	КонецЕсли;
	
	Возврат Ложь;
КонецФункции //-- ШмелевР 11.03.2026 ПРЗ.3 
