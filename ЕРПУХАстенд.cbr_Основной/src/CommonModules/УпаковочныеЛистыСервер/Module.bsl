...
Функция cbr_СинхронизироватьУпаковочныеЛистыСРасходнымОрдером(ОрдерСсылка, ИспользоватьАдресноеХранение) Экспорт
	...
	Если ОрдерСсылка.ТоварыПоРаспоряжениям.Количество() > 0 И ОбщегоНазначения.ЕстьРеквизитОбъекта("Контрагент", ОрдерСсылка.ТоварыПоРаспоряжениям[0].Распоряжение.Метаданные()) Тогда 
