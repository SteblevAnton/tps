...
Процедура ТоварыТЧЦенаСоСкидкойПриИзмененииНаСервере()
	...
	Для Каждого Стр Из Объект.Товары Цикл
		...
		Если Стр.cbr_СтрокаИзСоглашенияСКлиентом Тогда
			Стр.cbr_ЦенаСоСкидкой = Стр.Цена;
		...
		СтрокаКИзменению = СтрокиИзСоглашения.НайтиСтроки(Новый Структура("Номенклатура, Характеристика, УпаковкаСтарт",
			Стр.Номенклатура, Стр.Характеристика, Стр.Упаковка));	
		//--AlopinAs 170326 ПРД20
		...
		Если Стр.cbr_ЦенаСоСкидкой = 0 И Стр.Сумма <> Стр.cbr_ЦенаСоСкидкой * Стр.КоличествоУпаковок Тогда
...
Функция ПолучитьСтрокиКИзменениюИзСоглашения()
	...
	Запрос.Текст =
		...
		|	&СоглашениеСсылка КАК Соглашение,
		...
		|	ВЫБОР
		|		КОГДА ВТТЧ.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
		|			ТОГДА ВТТЧ.Номенклатура.ЕдиницаИзмерения
		|		ИНАЧЕ ВТТЧ.Упаковка
		|	КОНЕЦ КАК Упаковка,
		|	ВТТЧ.Соглашение КАК Соглашение,
		|	ВТТЧ.Дата КАК Дата,
		|	ВТТЧ.Упаковка КАК УпаковкаСтарт
		|ПОМЕСТИТЬ ВТТЧ2
		|ИЗ
		|	ВТТЧ КАК ВТТЧ
		|;
		...
		|	ВТТЧ2.УпаковкаСтарт КАК УпаковкаСтарт
		|ИЗ
		|	Справочник.СоглашенияСКлиентами.cbr_ЦеныПоТоварам КАК СоглашенияСКлиентамиcbr_ЦеныПоТоварам
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТТЧ2 КАК ВТТЧ2
		|		ПО СоглашенияСКлиентамиcbr_ЦеныПоТоварам.Номенклатура = ВТТЧ2.Номенклатура
		|		И СоглашенияСКлиентамиcbr_ЦеныПоТоварам.Характеристика = ВТТЧ2.Характеристика
		|		И СоглашенияСКлиентамиcbr_ЦеныПоТоварам.ЕдиницаИзмерения = ВТТЧ2.Упаковка
		|		И СоглашенияСКлиентамиcbr_ЦеныПоТоварам.Ссылка = ВТТЧ2.Соглашение
		|		И ВТТЧ2.Дата
...
&ИзменениеИКонтроль("ДатаПриИзмененииЗавершение")
&НаКлиенте
Процедура cbr_ДатаПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура ОбработатьИзменениеДаты()
	...
	Для Каждого Стр Из Объект.Товары Цикл
		...
		Иначе
			Стр.cbr_ЦенаСоСкидкой = Стр.Цена;
		КонецЕсли;
		Стр.Сумма = Стр.КоличествоУпаковок * Стр.cbr_ЦенаСоСкидкой;
...
&НаСервере
Функция cbr_СоглашениеПриИзмененииСервер(ПересчитыватьЦены)
...
&НаСервере
Процедура cbr_ПриЗаписиНаСервереПосле(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	
	//++ БаженоваЕО 10.03.2026 ПРД.21
	ТекущийОбъект.cbr_МаксимальнаяРучнаяСкидка = 0;
	Для Каждого СтрТЧ Из ТекущийОбъект.Товары Цикл
		Если Не СтрТЧ.cbr_ПоСоглашению Тогда
			ТекущийОбъект.cbr_МаксимальнаяРучнаяСкидка = Макс(ТекущийОбъект.cbr_МаксимальнаяРучнаяСкидка, СтрТЧ.ПроцентРучнойСкидки);
		КонецЕсли;
	КонецЦикла;
	...
	Пока Выборка.Следующий() Цикл
		ТекущийОбъект.cbr_ПроцентТУ = Выборка.ПроцентТУ;
	КонецЦикла;
	...
	Соглашение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущийОбъект.Ссылка, "Соглашение");
	СуммаПроцентАВСоглашения = Соглашение.cbr_АгентскиеВознаграждения.Итог("СтавкаАВ");
	СуммаПроцентАВЗаказа = ТекущийОбъект.cbr_АгентскиеВознаграждения.Итог("СтавкаАВ");
	
	ТекущийОбъект.cbr_СовпадениеСуммПроцентаАВ = Ложь;
	
	Если СуммаПроцентАВСоглашения = СуммаПроцентАВЗаказа Тогда
		ТекущийОбъект.cbr_СовпадениеСуммПроцентаАВ = Истина;
		ТекущийОбъект.cbr_ПроцентАВ = 1;
	ИначеЕсли СуммаПроцентАВЗаказа < СуммаПроцентАВСоглашения Тогда
		ТекущийОбъект.cbr_ПроцентАВ = 1;
	КонецЕсли;
	//-- БаженоваЕО 10.03.2026
...
Процедура cbr_ОбработатьНоменклатуру(ТекущаяСтрока)
	...
	СтруктураПарамтеров.Вставить("ДатаДокумента", Объект.Дата);
	...
	Если СтрокаТчЦеныПоТоварам = Неопределено Тогда
		ТекущаяСтрокаТч.cbr_ЦенаСоСкидкой = ТекущаяСтрокаТч.Цена;
		Возврат;
	КонецЕсли;
...
Функция ПолучитьТЧЦеныПоМассивуТоваров()
	...
	Запрос.Текст =
		...
		|		И &ДатаДокумента МЕЖДУ ВТСоглашение.ДатаНачала И ВТСоглашение.ДатаОкончания";
...
Функция ПолучитьТЧЦеныПоТоварам(СтруктураПарамтеров)
	...
	КонецЕсли;	
		...
		Запрос.УстановитьПараметр("ДатаДокумента", СтруктураПарамтеров.ДатаДокумента);
