...
Функция cbr_ТекстЗапросаДинамическогоСпискаДиспетчированиеЭтапов()
	...
	#Вставка
	// НАЧАЛО Османов Н.М. 28.11.2023 ЧТЗ №2 Управление производством
	|	Этапы.РабочийЦентр КАК РабочийЦентр,
...
Процедура ПодготовитьФорму()
	...
	Команда = ФормыУХ.СоздатьКоманду(ЭтаФорма, ИмяКомандыВперед, "", ИмяКомандыВперед);
	ИмяКомандыНазад = "П3_ПереместитьОчередьНазад";
	Команда = ФормыУХ.СоздатьКоманду(ЭтаФорма, ИмяКомандыНазад, "", ИмяКомандыНазад);
...
Процедура ОтборПоРабочемуЦентруПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(ЭтаФорма["ОтборПоРабочемуЦентру"]) Тогда
		УстановитьОтбор = Ложь;
	КонецЕсли;
	
	ОтборыСписковКлиентСервер.ИзменитьЭлементОтбораСписка(Список, 
														"РабочийЦентр",
														ЭтаФорма["ОтборПоРабочемуЦентру"],
														УстановитьОтбор,
...
Процедура ОтборДатыНаработкиПриИзменении(Элемент)
	...
	УстановитьОтбор = Истина;
	Если Не ЗначениеЗаполнено(ЭтаФорма["ОтборДатаНаработки"]) Тогда
		УстановитьОтбор = Ложь;
	КонецЕсли;
	
	Период = ЭтаФорма["ОтборДатаНаработки"];
