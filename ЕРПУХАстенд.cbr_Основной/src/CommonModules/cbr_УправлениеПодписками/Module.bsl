...
//++ Мохов 28.02.2026 ПРЗ.19
Процедура cbr_ПодпискаНаПроведенияДокументовЛИМСОбработкаПроведения(Источник, Отказ, РежимПроведения) Экспорт	
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.лимсЗаявкаНаКонтроль") Тогда
		...
		Если РезультатЗапроса.Следующий() Тогда
			Попытка
				...
				ДокументЭтапОбъект.cbr_ДатаНачалоКонтроля = Перечисления.cbr_СтаутсыКонтроляЭтаповПроизводства.КонтрольНачат;
	...
	ИначеЕсли ТипЗнч(Источник)	= Тип("ДокументОбъект.лимсРегистрацияРезультатовКонтроля") Тогда
		...
		Если РезультатЗапроса.Следующий() Тогда 
			Попытка
				ДокументЭтапСсылка = РезультатЗапроса.ДокументОснование;
				ДокументЭтапОбъект = ДокументЭтапСсылка.ПоулчитьОбъект();
				ДокументЭтапОбъект.cbr_СтатусКонтроля = Перечисления.cbr_СтаутсыКонтроляЭтаповПроизводства.КонтрольОкончен;
				ДокументЭтапОбъект.Записать();
			Исключение
		...
		Если Источник.КоличествоНесоответствующейПродукции = Источник.ДокументОснование.cbr_Брак Тогда
			Возврат; 	
		КонецЕсли;
		...
		ДокументОбъект = Источник.ДокументОснование.ПолучитьОбъект();
		
		Если ДокументОбъект.ВыходныеИзделия.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
		
		ИтоговаяТаблица = ДокументОбъект.ВыходныеИзделия.Выгрузить();
		ИтоговаяТаблица.Свернуть(
		"Номенклатура, Характеристика, Упаковка, Получатель, Назначение, ДатаПроизводства, СтатусУказанияСерий, СтатусУказанияСерийОтправитель, СтатусУказанияСерийПолучатель, ДоляСтоимости, ДоляСтоимостиНаЕдиницу, СписатьНаРасходы, СтатьяРасходов, АналитикаРасходов, АналитикаАктивовПассивов, АналитикаУчетаНоменклатуры, ОформитьОтчетДавальцу, ОбработатьПоСпецификации, Спецификация, ЭтапПотребитель, Период, Подразделение, НастройкаСчетовУчета, ИспользуетсяАвтовыбор, СведенияАвтовыбора",
		"КоличествоУпаковок, Количество, Цена, Сумма"); 
		
		ДокументОбъект.ВыходныеИзделия.Очистить();
		НоваяСтрокаТЧ = ДокументОбъект.ВыходныеИзделия.Добавить(); 
		ЗаполнитьЗначенияСвойств(НоваяСтрокаТЧ, ИтоговаяТаблица[0]);
		
		ДокументОбъект.ОтразитьКоличествоОтмененоВТЧ(ПричинаОтмены);
		ДокументОбъект.ОтразитьКоличествоФактВТЧ(Неопределено);
...
КонецПроцедуры //-- Мохов 28.02.2026 ПРЗ.19
